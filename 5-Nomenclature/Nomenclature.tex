...
\nomenclature[G]{$\mu_k$, $\mu_k(t)$, $\mu_k^j$, $\mu_i^{(k)}$}{Mean of arm $k$, arm $k$ at time $t$ (in Chapter~\ref{chapter:6}), arm $k$ for user $j$ (in Chapter~\ref{chapter:5}), arm $i$ in the $k$-th stationary interval (in Chapter~\ref{chapter:6})}
...
\nomenclature[G]{$\tau_k^j$, $\tau^{(k)}$}{Location of a change-point, \eg, the $j$-th change-point on arm $k$, in Chapter~\ref{chapter:6}}
...
\nomenclature[G]{$\varepsilon$}{Usually denotes a small positive real value, \eg, the parameter for the $\varepsilon$-greedy algorithm, or a drift correction parameter for the \CUSUM{} algorithm in Chapter~\ref{chapter:6}}
...
\nomenclature[G]{$\pi$}{It usually denotes the number $\pi\simeq 3.14\ldots$, but it also denotes a probability distribution in Section~\ref{sec:2:chooseYourPreferredBanditAlgorithm} or a permutation in Section~\ref{par:5:twoIdeasOrthogonalization}}
...
\nomenclature[L]{$\cA$}{An algorithm, also referred to as a policy or a strategy. $\cA_1,\dots,\cA_N$ denote the $N$ aggregated algorithms in Section~\ref{sec:2:chooseYourPreferredBanditAlgorithm} and $\cA_1,\dots,\cA_M$ denotes the algorithms of the $M$ players in Chapter~\ref{chapter:5}}
...
\nomenclature[L]{$O_t$}{Vector of observations until time $t$ in Chapter~\ref{chapter:5}}
...
\nomenclature[L]{$C_k(t)$, $C^j(t)$}{In Chapters~\ref{chapter:4} and \ref{chapter:5}, collision indicator, on arm $k\in[K]$ or for user $j\in[M]$, at time $j$}
...
\nomenclature[I]{$Y_{k,t}$}{Usually denotes a variable depending on an arm, for $k\in[K]$, and on time, for $t\in[T]$}
...
\nomenclature[E]{$y^{(k)}$, $y^{(\ell)}$}{Usually the superscript index $(k)$ or $(\ell)$ denotes a variable in the $k$-th stationary interval, under the  point-of-views of \emph{global} or \emph{local} changes, in Chapter~\ref{chapter:6}}

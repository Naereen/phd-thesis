...
\nomenclature[G]{$\tau$}{Usually denotes the length of a sliding-window algorithm like Sliding-Window-\UCB{} in Chapter~\ref{chapter:6}}
\nomenclature[G]{$\gamma$}{Usually denotes the discount factor of an algorithm like Discounted-\UCB{} in Chapter~\ref{chapter:6}}
...
\nomenclature[O]{$X'$}{Usually denotes a ``wrong value'' of a variable $X$, for instance $M'$ denotes in Section~\ref{par:5:usingWrongValueofM}}
...
\nomenclature[I]{$x_k$}{Usually the subscript index $k$ is denotes a variable depending on an arm, for $k\in\{1,\dots,K\}$}


% --------------------------------
% matches Superscripts         > E
\nomenclature[E]{$y^j$}{Usually the superscript index $j$ is denotes a variable depending on a player, for $j\in\{1,\dots,M\}$}
\nomenclature[E]{$y^{(k)}$, $y^{(\ell)}$}{Usually the superscript index $(k)$ (resp. $(\ell)$) is denotes a variable in the $k$-th stationary interval, under the \emph{global} (resp. \emph{local}) changes point-of-view, in Chapter~\ref{chapter:6}}

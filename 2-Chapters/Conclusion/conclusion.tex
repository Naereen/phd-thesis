...
%
For the last two chapters, the proposed algorithms attain state-of-the-art performances both on numerical simulations and on the regret upper bounds obtained in our theoretical analyses.
% \end{itemize}
...
%
This first part ends by presenting one of our first contributions \cite{Besson2018WCNC}, in Chapter~\ref{chapter:25}.
...
We believe that our new proof technique of Chapter~\ref{chapter:6} could be used to analyze \GLRklUCB{} under less stringent assumptions than the one made in this chapter (and in previous works), that would require only a few ``meaningful'' changes to be detected.
...
Another interesting question is to look for a unique ``unified'' algorithm that can be used in all the different settings studied in this thesis, and automatically adapt to the setting at hand.

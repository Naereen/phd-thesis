...
It presented the problems we considered for my PhD research, and we discussed about the main contributions produced during the last three years.
...
%
Finally, we presented in Chapter~\ref{chapter:25} a first mathematical contribution, our algorithm \Aggr, as an efficient answer to the online algorithm selection problem.
...
% Chapter 5
Because the models studied for realistic IoT networks were found to be intractable to analyze from a theoretical point of view, mainly due to the large number of algorithms interacting at random times in an unpredictable environment,
...
% Chapter 6
Finally, in Chapter~\ref{chapter:6}, we removed the stationary hypothesis on the single-player MAB problem, and we analyzed a new actively adaptive algorithm, for which we also proved state-of-the-art results (\GLRklUCB).
%
...
Even if SMPyBandits is still restricted to the finite-arm case, it supports a wide range of arm distributions (as well as rested or restless Markov models).
...
Similarly, if the number of devices remain constant, increasing the successful transmission rate directly improves the network QoS.
...
%
By using such low-cost algorithms, we demonstrated empirically that a dynamically re-configurable device can learn on its own to favor a certain channel, if the environment traffic is not uniform between the $K$ different channels, by using onlt the acknowledgement (\Ack) feedback sent from the base station.


Finally in Section~\ref{sec:4:retransmissions}, we presented an extension of our model of LPWA networks based on an ALOHA protocol, again under the hypothesis of a protocol slotted in time and frequency and with perfect synchronisation.
...
Surprisingly, the main take-away message is that a simple \UCB{} learning approach (that simply retransmits in the same channel), turns out to perform almost as well as more complicated heuristics.
...
is interesting for its simplicity, and because considering no retransmission can improve the battery life of the IoT devices, at the cost of a lower Quality of Service (QoS).
%

                  ...
                  \eIf{$A^j(t) \notin \TopM(t)$}{
                    \eIf(\tcp*[f]{collision}){$C^j(t)$}{
                      ...
                      \tcp*[f]{randomly switch}
                        ...
                        $A^j(t+1) \sim \cU \left(\TopM(t) \cap \left\{k : g_k^j(t-1) \leq U^j_{A^j(t)}(t-1)\right\}\right)$
                      }
                    }{
                      $A^j(t+1) = A^j(t)$
...
\begin{smallproof}
  ...
  &= \sum_{t=1}^T \Pr_{\bm{\mu}} \left( A^j(t) = k,\;\; \exists m \in[M]:\; U_{m^*}^j(t) < g_k^j(t) \right).
  %
  \end{align*}
  ...
  \begin{align}
    ...
    \;\;\;\;
    \sum_{t=1}^T\Pr_{\bm{\mu}} & \left(A^j(t) = k,\;\; \exists m_2 \in [M]:\; U_{m_2^*}(t) \leq g_k(t) , \forall {m_3} \in [M]: \;  U_{{m_3}^*}(t) \geq \mu_{m_3}^* \right) \notag \\
    &\leq \sum_{m_1=1}^M\sum_{t=1}^T\Pr_{\bm{\mu}} \left(U_{m_1^*}(t) < \mu_{m_1}^* \right) + \sum_{t=1}^T\Pr_{\bm{\mu}} \left(A^j(t) = k,\;\; \exists m_2 \in [M]:\; \mu_{m_2}^* \leq g_k(t)\right) \notag \\
    %

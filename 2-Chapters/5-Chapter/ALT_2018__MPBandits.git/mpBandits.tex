...
\begin{leftbar}[defnbar]  % XXX leftbar defnbar, comment if needed
  ...
  Let $T^j_k(T) \eqdef \sum_{t=1}^T \indic(A^j(t) = k)$,
  and denote $T_k(T) \eqdef \sum_{j=1}^M T^j_k(T)$ the \emph{number of selections} of arm $k\in[K]$ by any player $j\in[M]$, up to time $T$.
...
%
Letting $\cP_M \eqdef \left\{ \boldsymbol{\mu} \in [0,1]^K : \mu_M^* > \mu_{M+1}^*\right\}$
...
\begin{equation}
  ...
  \ \ \ \text{and letting} \ \ \ \widetilde{\mu_k}^j(t) \eqdef \widetilde{S_k}^j(t) \;/\; T_k^j(t).
\end{equation}
...
\begin{smallproof}
  ...
  \begin{align*}
    ...
    & = \sum_{k' \neq k} \underbrace{\sum_{t=1}^T \Pr\left(A^j(t) = k,A^j(t+1) = k', U_{k'}^j(t) \geq U_{k}^j(t), U_{k'}^j(t-1) \leq U_{k}^j(t-1)  \right)}_{ \eqdef T_{k'}}
  \end{align*}

...
As we state it here, the constraint on fairness is new, and it was not studied much in previous works.
...
We note that only a very recent article studies this measure of performance under a formal approach, \cite{Patil2019stochastic}, but only for the single-player case and therefore they consider a slightly different notion of fairness.
...
Fix a player $j\in[M]$, and we consider the observations $O_t$ gathered by this player after $t$ rounds of its algorithm $\rho^j$:
...
We denote $\Pr_{\bm{\mu}}^{O_t}$ the distribution of the vector $O_t$ under the model $\bm{\mu}$ when using algorithm $\rho$.
Then we introduce the \emph{collision information term} as:
\begin{equation}\label{eq:5:collisionInformationTerm}
  \cI_{\bm{\mu},\bm{\lambda}}(\rho,T) := \sum_{t=1}^{T} \mathrm{KL}(\Pr_{\bm{\mu}}^{C_t | O_{t-1}}, \Pr_{\bm{\lambda}}^{C_t | O_{t-1}}).
\end{equation}
...
We report in Table~\ref{table:5:comparisonUCB_klUCB} below the results in terms of mean regret, plus or minus one standard deviation.

...
For the development of Cognitive Radio, a crucial step is to insert \emph{multiple} smart devices in the \emph{same} background traffic (at least $M \geq 2$).
...
Other models for rewards loss have been proposed in the literature, for instance the reward is randomly allocated to one of the players selecting it, but we focus on full reward occlusion in this chapter.


A multi-player MAB strategy is a tuple $\rho = (\rho^1,\dots,\rho^M)$ of arm selection strategies for of each of the $M$ players, and the goal is to propose a strategy that maximizes the total reward of the system, under some constraints.
...
%
Practitioners may still be interested by the algorithm, especially for Cognitive Radio applications in which sensing is hard or not considered.
...
% -----------------------------------------------------------------
\section{Theoretical elements and order optimal regret upper bound for \MCTopM}
...
%
We note that introducing \emph{switching costs}, like it was done in previous works like \cite{Koren17}, could be an interesting future work.

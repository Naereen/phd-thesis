...
%
\begin{equation}
  A^j(t) = \argmax_{ k \in [K]} \ \widetilde{U_k}^j(t-1).\label{algo:5:Selfish}
\end{equation}
...
\begin{equation}\label{eq:5:indexFor_UCB_klUCB}
  ...
  \end{cases}
\end{equation}
where $f(t)$ is some \emph{exploration function}. $f(t)$ is usually taken to be $\log(t)$ in practice, and slightly larger in theory, which ensures that  $\Pr(U_k^j(t) \geq \mu_k) \gtrsim 1 - 1/t$ (see \cite{KLUCBJournal}).

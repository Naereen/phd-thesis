...
\begin{smallproof}
  ...
  \begin{leftbar}[lemmabar]  % XXX leftbar lemmabar, comment if needed
      ...
      % \begin{equation}
      $\sum\limits_{t=1}^T \Pr_{\bm{\mu}} \left(U_k^{j}(t) < \mu_k\right) \leq 3 + 4 \mathrm{e} \log(\log(T))$.
      % \end{equation}
  \end{leftbar}  % XXX leftbar lemmabar, comment if needed
  \end{lemma}
...
\end{lemma}
  ...
  \begin{align}\label{eq:5:UBprecise}
    ...
    & \;\;\;\;\;\;+ 2\left(\frac{\kl'(\mu_k,\mu_{M^*})}{\kl(\mu_k,\mu_{M^*})}\right)^2 + 4M \mathrm{e} \log(\log(T)) + 3M+1,
  \end{align}
...
Our bound is worse than the $\bigO{M^2}$ proven by \cite{Rosenski16} for \MusicalChair{}. %, due to our trick of focusing on collisions for non-sitted players.

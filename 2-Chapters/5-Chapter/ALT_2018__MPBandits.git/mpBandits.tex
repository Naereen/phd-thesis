...
%
\begin{equation}
  A^j(t) = \argmax_{ k \in \{1,\dots,K\}} \ \widetilde{g_k}^j(t-1).\label{algo:5:Selfish}
\end{equation}
...
\begin{proof}
  ...
  \begin{align}
    \E_{\mu}[T_k^j(T)] &\leq
    \sum_{t=1}^T\Pr_{\bm{\mu}} \left( \exists m_1 \in \{1,\dots,M\}: \; g_{m_1^*}(t) < \mu_m^* \right) + \notag \\
    \;\;\;\;
    \sum_{t=1}^T\Pr_{\bm{\mu}} & \left(A^j(t) = k,\;\; \exists m_2 \in \{1,\dots,M\}:\; g_{m_2^*}(t) \leq g_k(t) , \forall {m_3} \in \{1,\dots,M\}: \;  g_{{m_3}^*}(t) \geq \mu_{m_3}^* \right) \notag \\
    &\leq \sum_{m_1=1}^M\sum_{t=1}^T\Pr_{\bm{\mu}} \left(g_{m_1^*}(t) < \mu_{m_1}^* \right) + \sum_{t=1}^T\Pr_{\bm{\mu}} \left(A^j(t) = k,\;\; \exists m_2 \in \{1,\dots,M\}:\; \mu_{m_2}^* \leq g_k(t)\right) \notag \\
    %

...
\begin{smallproof}
  ...
  but it is not obvious for \ref{eq:5:term2} as there is no reason for $N_k(T)$ to be upper bounded by $T$ (it counts the selections of arm $k$ by \emph{all} the players, and for instance if player $1$ is fixed on arm $1$ and player $2$ plays it at least once, then $N_1(T)\geqT+1$).
  %
      ...
      \begin{algorithm}[H]
          ...
          % XXX Algorithm
              ...
              \For{$t = 1, \dots, T$}{
                  %
                  \eIf{$A^j(t-1) \notin \TopM(t)$}{
                    \eIf(\tcp*[f]{collision at previous step}){$C^j(t-1)$}{

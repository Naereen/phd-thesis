...
Finally, the result of our experimental study reported in Section~\ref{sec:5:experiments}.
...
\begin{framed}
  ...
  but the results stated in the section are now correct,
...
To express our lower bound, we need to introduce $\kl(x,y)$ as the Kullback-Leibler divergence between the Bernoulli distribution of mean $x$ and that of mean $y$, so that $\kl(x,y) := x\log(x/y) + (1-x)\log((1-x)/(1-y))$.
%
...
However, we wanted to be still able to present our derivations and results giving the desired lower-bound, mainly because its proof and its form give insights on how to design an asymptotically optimal algorithm $\rho$, and because it still applies to algorithms of the \RhoRand{} family as well as our proposals \RandTopM{} and \MCTopM.
...
% FIXME
% We let the interested reader fetch the details on their paper.
...
For the algorithm $\rho$, we introduce the \emph{collision information term} as:
\begin{equation}\label{eq:5:collisionInformationTerm}
  \cI_{\bm{\mu},\bm{\lambda}}(\rho,T) := \sum_{t=1}^{T} \mathrm{KL}(\P_{\bm{\mu}}^{C_t | \cO_{t-1}}, \P_{\bm{\lambda}}^{C_t | \cO_{t-1}}).
\end{equation}

We now state a problem-dependent asymptotic lower bound on the number of sub-optimal arms selections under a \emph{decentralized strategy} $\rho$ that has access to the sensing information and satisfies $\cI_{\bm{\mu},\bm{\lambda}}(\rho,T) = \smallO{\log(T)}$.
This additional hypothesis on $\cI_{\bm{\mu},\bm{\lambda}}(\rho,T)$ essentially says that ``the collisions do not bring too much information on the arm means'', as stated in \cite{KaufmannAbbas19}.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    %
    & \leq \sum_{t=1}^T \Pr\left( A^j(t) = k,\; \mu_{M^*} \leq g_k(t) \right)
    + \sum_{m=1}^M \sum_{t=1}^T \Pr\left( g_{m^*}(t) < \mu_m^* \right)
    \hspace*{100pt} (\heartsuit)
  \end{align*}
...
\begin{proof}
  ...
  \begin{align}
    ...
    & \leq \sum_{s=1}^T \bP\left( s \times \kl^+\left(\widehat{\mu}_{k,s}^j , \mu_k\right) \leq f(T)\right),\label{eq:5:FromHere}
  \end{align}

...
% &= \sum_{t=1}^T \Pr\left( A^j(t) = k,\;\; \exists m \in[M]:\; U_{m^*}^j(t) < g_k^j(t) \right).
% %
...
% \E_{\mu}[T_k^j(T)] &\leq \sum_{t=1}^T\Pr\left(A^j(t) = k,\;\; \exists m \in[M]:\; U_{m^*}(t) \leq g_k(t) , \forall m \in [M]: \;  U_{m^*}(t) \geq \mu_m^* \right)\\
% &\;\;\;\;\;\;\;\; + \sum_{t=1}^T\Pr\left( \exists m \in[M]: \; U_{m^*}(t) < \mu_m^* \right) \\
% &\leq \sum_{t=1}^T\Pr\left(A^j(t) = k,\;\; \exists m \in[M]:\; \mu_{m}^* \leq g_k(t)\right)+ \sum_{m=1}^M\sum_{t=1}^T\Pr\left(U_{m^*}(t) < \mu_m^* \right) \\
% %
% &\leq
% \sum_{t=1}^T
...
% $U_{k'}^j(t-1) \leq g_k^j(t-1), A^j(t-1)=k$
% and
% $g_k^j(t) \leq U_{k'}^j(t), A^j(t)=k'$.
% %
% In particular,
% if $\mu_k \geq \mu_{k'}$,
% $\phi_{k,k'}^j(t)$ implies $g_k^j(t) \leq U_{k'}^j(t)$,
% and
% if $\mu_k < \mu_{k'}$, then
% $\phi_{k,k'}^j(t)$ implies $U_{k'}^j(t-1) \leq g_k^j(t-1)$,
...
% Then, essentially $g_k^j(t) \leq U_{k'}^j(t)$
...
%     &\leq
%     \sum_{t=1}^{T} \Pr\left(g_k^j(t) \leq U_{k'}^j(t), A^j(t-1)=k, A^j(t)=k'\right)
%     \intertext{Either $U_{k'}^j(t) > \mu_k$ or $U_{k'}^j(t) \leq \mu_k$, and by introducing the two cases,}
%     &\leq
%     \sum_{t=1}^{T} \Pr\left(g_k^j(t) \leq U_{k'}^j(t) \leq \mu_k, A^j(t-1)=k, A^j(t)=k'\right) \notag \\
%     &\;\;+ \sum_{t=1}^{T} \Pr\left(g_k^j(t) \leq U_{k'}^j(t), U_{k'}^j(t) > \mu_k, A^j(t-1)=k, A^j(t)=k'\right) \notag
%     \intertext{For the first term, $g_k^j(t) \leq \mu_k$ is highly unlikely (\ie, the sum of probabilities is $\smallO{\log T}$) as the indices ${(g_k^j)}_k$ are Upper Confidence Bounds (if arm $k$ is sampled enough). For the second term, $U_{k'}^j(t) > \mu_k$ is unlikely if arm $k'$ is sampled enough. More formally,}
%     \sum_{t=1}^{T} \Pr(\phi_{k,k'}^j(t))
%     &\leq
...
% or whether $g_k^j(t-1) \leq \mu_k$ which implies $U_{k'}^j(t-1) \leq \mu_{k'}$ (and which is also highly unlikely), and with Lemmas~\ref{lem:5:Fact1KLUCB}, \ref{lem:5:Fact2KLUCB},
% we obtain that

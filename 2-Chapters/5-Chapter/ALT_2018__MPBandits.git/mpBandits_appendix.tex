...
% &= \sum_{t=1}^T \Pr\left( A^j(t) = k,\;\; \exists m \in[M]:\; g_{m^*}^j(t) < g_k^j(t) \right).
% %
...
% \E_{\mu}[T_k^j(T)] &\leq \sum_{t=1}^T\Pr\left(A^j(t) = k,\;\; \exists m \in[M]:\; g_{m^*}(t) \leq g_k(t) , \forall m \in [M]: \;  g_{m^*}(t) \geq \mu_m^* \right)\\
% &\;\;\;\;\;\;\;\; + \sum_{t=1}^T\Pr\left( \exists m \in[M]: \; g_{m^*}(t) < \mu_m^* \right) \\
% &\leq \sum_{t=1}^T\Pr\left(A^j(t) = k,\;\; \exists m \in[M]:\; \mu_{m}^* \leq g_k(t)\right)+ \sum_{m=1}^M\sum_{t=1}^T\Pr\left(g_{m^*}(t) < \mu_m^* \right) \\
% %
% &\leq
% \sum_{t=1}^T

...
% Using the same technique as \cite{Garivier16TrueShape}, called the ``contraction of entropy'' principle, one can establish that for any event $A$ that is $\sigma(\cO_t)$-measurable\footnote{In the work of \cite{Garivier16TrueShape}, the statement is more general and the probability of an event $A$ is replaced by the expectation of any $\cF_T$-measurable random variable $Z$ bounded in $[0,1]$.},
...
the number of players, from $1$ player to $9$ players, and on the
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.90\textwidth]{Comparison_of_our_lowerbound_and_Anandkumar_2010__9_arms.pdf}
  \caption[Comparison of our lower bound against the one from \cite{Zhao10}.]{Comparison of \textcolor{red}{our lower bound (in red)} against \textcolor{blue}{the one from \cite{Zhao10} (in blue)}, on a small problem with $9$ Bernoulli arms, of means $\boldsymbol{\mu} = [0.1, \dots, 0.9]$, as a function of the number of players $M$.}
...
We considered $1000$ independent runs and plot the average regret.
%
...
%
These plots show the regret (in red),
...
\begin{figure}[!h]
    ...
    % \end{subfigure}
    \caption[Illustrating the decomposition of the multi-players regret with its three terms.]{The three terms \ref{eq:5:term1} (\textcolor{red}{in red}), \ref{eq:5:term2} (\textcolor{cyan}{in cyan}), \ref{eq:5:term3} (\textcolor{green}{in green}) of the multi-players regret, and lower bounds \eqref{eq:5:ourLowerBound} and \eqref{eq:5:Zhao10LowerBound} (\textbf{in black}), for \Selfish-\klUCB. Two cases for $M=6$ and $M=9$ players, $K=9$ arms, horizon $T=10000$.}

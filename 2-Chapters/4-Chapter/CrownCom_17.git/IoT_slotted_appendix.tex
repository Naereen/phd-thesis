...
\begin{itemize}
    ...
    So the gradient of the Lagrangian is zero iff $D_i^* = \max(0, x/\log(1-p)) = \left[ \frac{1}{\log(1-p)}\mathcal{W}(\lambda\mathrm{e} / (1-p)^{S_i - 1}) - 1\right]^{+}$, because $D_i^*$ has to be non-negative.

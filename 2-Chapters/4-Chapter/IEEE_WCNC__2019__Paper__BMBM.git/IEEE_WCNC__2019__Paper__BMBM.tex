...
In this section, we extend the model presented before, to take into account the possibility for retransmissions of a message after a collision.
This extension is of major importance: most protocols for real-world IoT networks can use retransmissions, it is for instance the case of the LoRa standard.
...
\begin{figure}[htp!]  % [htbp]
	...
	\includegraphics[width=0.80\linewidth]{Approximation_m10-eps-converted-to.pdf}
...
% 	A(t) = \arg\max_{1\leq k \leq K} U_k(t).
% \end{equation}
...
% 			Transmit in channel $A(t) = \arg\max_{1\leq k \leq K} U_k(t)$\;
...
% ----------------------------------------------------------------------
\subsection{The first heuristics: \UCB{} unaware of retransmissions}
...
The \UCB{} algorithm is implemented independently by each device, and we present it in Algorithm~\ref{algo:43:UCB}.
It is the same algorithm as the one presented in Chapter~\ref{chapter:2}, in Section~\ref{sub:2:IndexPolicies} in Algorithm~\ref{algo:2:indexPolicy} and \eqref{eq:UCB_index},
but we write it again to make clear the difference with first transmission and retransmission of messages.
%
Note that a device using this first approach is only able to select a channel for the first transmission (using \UCB), and it uses the same channel for all the corresponding retransmissions of a packet (if retransmissions happen).
...
The empirical mean estimator $\widehat{\mu_k}(t)$ of channel $k$ is defined as the mean reward obtained up-to time $t-1$,
$\widehat{\mu_k}(t) = \frac{1}{N_k(t)} \sum_{\tau=0}^{t-1} r_k(\tau) \mathbbm{1}(A(\tau) = k)$,
where $r_{k}(t)$ is the reward obtained after transmission in channel $k$ at time $t$.
...
\begin{figure}[h!]
	...
	\begin{algorithm}[H]
		...
		\caption{First-stage \UCB{} and retransmission in same channel.}
		\label{algo:43:UCB}
		% \end{small} % XXX remove if needed
	\end{algorithm}
	\end{framed}
...
\paragraph{\textcolor{red}{Small warning about notations: local (device) time vs global (gateway) time}.}
%
In all the algorithms and notations presented in this Section, the time steps $t$ do not refer to the global time steps (as seen from the gateway), but rather the current number of uplink transmissions (or retransmissions) that was carried out by the device.
In other words, each device wakes up whenever it has to send some data, following its random emission process (here, we propose a Bernoulli process of small probability, \eg, $p=10^{-3}$), and when it woke up, it sends its packet, and will send again for at most $M$ times (\eg, $M=10$) until it receives an \Ack.

To be clear, once again, we repeat that the time steps $t$ in the following algorithms denote the number of time the device sent an uplink packet, and these uplink (re)transmissions can happen at different (real-world) times, more or less spread out in time, it does not matter to the learning part of this model and work.
	...
	\begin{algorithm}[H]
		...
		{$t = 1, \dots, T$}{
			...
			\Else(\tcp*[f]{Packet retransmission with $\mathrm{UCB}^j$}){ % Retr. after trying channel $j$
				...
				Transmit in channel $A^j(t) = \arg\max_{1\leq k \leq K} U^j_k(t)$\;
	...
	\begin{algorithm}[H]
		...
		{$t = 1, \dots, T$}{
			...
			\Else(\tcp*[f]{Packet retransmission with delayed $\mathrm{UCB}^d$}){
				...
				Transmit in channel $A^d(t) = \arg\max_{1\leq k \leq K} U^d_k(t)$\;
...
We believe that the same empirical results, but more importantly, the same conclusions, could be given if we used Bayes-UCB or other bandit algorithm for the base building block in the different heuristics proposed in Section~\ref{sub:43:heuristics}.
The goal of this Section was not to optimize on the bandit policy (as we presented in Section~\ref{sec:3:reviewSPAlgorithms} some numerical simulations doing precisely this), but rather to compare the different heuristics, for a fixed bandit policy (for which we preferred to use \UCB{} for simplicity).

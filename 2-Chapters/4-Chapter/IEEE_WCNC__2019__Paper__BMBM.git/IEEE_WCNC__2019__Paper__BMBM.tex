...
This extension is of major importance as most protocols for real-world IoT networks use retransmissions, and it is for instance the case of the LoRa standard.
...
the goal is not to minimize the number of retransmission, but to maximize the probability of successful transmission, considering both the first transmission of a message and the retransmissions of the same message, because the objective of each device is to maximize its battery life by minimizing its number of failed transmissions.
...
We make two approximations $\mathcal{H}_{1}$ and $\mathcal{H}_{2}$ defined as,
\begin{itemize}
	\item $\mathcal{H}_{1}$:
...
\begin{itemize}
	\item
	On one hand, in a congested network, when devices suffer from a large probability of collision on their first transmission (\ie, $p_c$ is not so small), then $p_{c1}\simeq p_c$ and so devices cannot really hope to reduce their collision probabilities even if the use a different channel for retransmission.
	\item
	...
	\begin{figure}[h!]
		...
		\begin{algorithm}[H]
			...
			{$t = 0, \dots, T$}{
				...
				\Else(\tcp*[f]{retransmission with delayed $\mathrm{UCB}^d$}){
...
A first result is that all the heuristics clearly outperform the non-learning approach that simply uses random channel selection for both transmissions and retransmissions (\ie, the ``\textbf{no \UCB{}}'' curve).

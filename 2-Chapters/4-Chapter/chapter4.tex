...
%
We propose two models of IoT networks, composed of many independent IoT end-devices, that can use low-cost Reinforcement Learning (RL) algorithms to learn to improve their spectrum access.
%
...
%
However, our second model considers that in case of failed transmission of a message, a dynamic device can also retransmit it up-to a fixed number of retransmission.
We compare different heuristics, based on \UCB, and numerical simulations confirm that the non-naive heuristics also significantly improve the network efficiency.
...
%  (WiFi, LTE-U), (IoT) standards for short-range (ZigBee, Z-Wave, Bluetooth) and long-range (LoRaWAN, SIGFOX, Ingenu, Weightless) communications \cite{Centenaro16}.
...
\newpage  % WARNING ?
% ----------------------------------------------------------------------------
\section{Towards theoretical guarantees}
% \section{Conclusion}
...
We presented two models of wireless IoT networks, without relying on the feedback provided by spectrum sensing, and inspired by the ALOHA protocol. We proposed two versions, with or without retransmissions of uplink packages in case of collisions.
...
That is why we focus on two different simplifications of this model in the next chapters, for which we are able to provide a clean theoretical analysis.
% %

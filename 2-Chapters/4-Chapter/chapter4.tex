...
Kaufmann12Thompson}, for the first model. For the demonstration as well as for the second model, we preferred to focus on heuristics based on the simplest algorithm (\ie, \UCB), to give a clear presentation of the different ideas explored for to solve the problem of learning to retransmit efficiently.
...
%
By using such low-cost algorithms, we demonstrated empirically that a dynamically re-configurable object can learn on its own to favor a certain channel, if the environment traffic is not uniform between the $K$ different channels.

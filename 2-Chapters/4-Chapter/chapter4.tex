...
We also present a proof-of-concept real-world validation of the advantage of using Multi-Armed Bandit algorithms for the first model, by showing and explaining a demonstration implemented on USRP cards \cite{USRPDocumentation}.

More precisely, on one hand, we start by motivating this idea of independent ``selfish'' learning for IoT devices in Section~\ref{sec:4:motivations}.
...
On the other hand, we present in Section~\ref{sec:4:retransmissions} a second model extending our first model, when devices can retransmit their messages for a fixed number of maximum retransmission (like in the ALOHA protocol).
...
Kaufmann12Thompson}, for the first model. For the demonstration and the second model, we preferred to focus on heuristics based on the simplest algorithm (\ie, \UCB), to give a clear presentation of the different ideas explored for to solve the problem of learning to retransmit efficiently.
...
Recently, TS and \UCB{} algorithms have been used for improving the spectrum access in (unlicensed) WiFi networks, for instance by \cite{Toldov16}.
...
We present in Section~\ref{sec:4:firstModel} how the MAB algorithms can be used in a unlicensed and frequency- and time-slotted IoT network \cite{Bonnefoi17}.
Several devices are using bandit algorithms, and the assumptions made by the stochastic bandit algorithms are not satisfied: as several agents learn simultaneously, their behavior is not stationary.
...
We then validate the model with a hardware implementation, detailed in Section~\ref{sec:4:gnuradio}.
%
We conclude this Chapter by presenting an extension of this model to take into account another aspect of the ALOHA protocol, that is the possibility for dynamic devices to retransmit their packet if the \emph{Ack} was not received.

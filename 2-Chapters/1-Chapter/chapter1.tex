...
two other famous and worldwide deployed examples of mono-directional radio systems are the long-range global navigation satellite systems (such as USA's GPS or European Union's Galileo), and short-range infrared remote controllers.
...
The first model considers IoT devices that simply have data to send to a base station, at a regular frequency (but random times), and that use acknowledgements as feedback, in order to optimize their uplink communications, by accessing more the best channels (\ie, the channel less occupied by the surrounding traffic).

...
As exposed in the definition above of the CR philosophy, we could imagine that if the network is not used by any PU in a certain location and time, then non licensed users, referred to as Secondary User (SU), could use it, by paying some rent to the PU.
%
Regulation states that PU have a strict priority,
but even though the RF bands are allocated, real world measurements often show that some bands are not densely used, and thus if a SU was equipped with an efficient sensing capacity, it could analyze its environment, and use a licensed band if it is free of any PU.
% explain DSA
...
and considers a fixed and finite set of orthogonal channels, \ie, different frequency bands in a licensed spectrum.
...
under partial information feedback (\ie, about one channel over $K$ at each time step).
...
Indeed, $r(t) = 1$ indicates that no Primary User was sensed (and thus a SU message can be sent), while a reward of $r(t)=0$ indicates that the channel $k$ is busy at time $t$ and no SU message should be sent, until the end of that slot.
%
...
along with the early works of Q. Zhao and her team, for instance with the works \cite{Liu08,Zhao10}.
%
...
some of their recent works are also illustrated with real-world demo using USRP and the MATLAB/Simulink system
...
\begin{itemize}\tightlist
    ...
    \item
    most IoT devices are very low-cost devices, possibly running on low-quality hardware, and having limited software and power capabilities (\ie, tiny batteries).
    It means that they even if they are not equipped with particular sensing capabilities,
    ...
    \item
    IoT devices have low duty cycles (only a few messages every minute) to very low duty cycles (only a few messages every month).
\end{itemize}

A central coordination of the devices with their base station is impossible as it would require to communicate continuously with the base station, which violates the two last constraints.
%

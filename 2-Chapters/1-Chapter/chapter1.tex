...
\paragraph{From Machine Learning to Reinforcement Learning.}
%
We can narrow down the first field of study of this thesis, steps by steps.
Starting from the general concept of Machine Learning (ML), we focused our interest to statistical learning, then sequential learning, then Reinforcement Learning (RL, \cite{SuttonBarto2018}), then RL with limited feedback and finally to multi-armed bandits learning.
...
Many real-world problems can be framed as Reinforcement Learning problems \cite{bouneffouf2019survey}, for instance learning to walk or to drive, learning to play a board or computer game, or discovering which treatment is efficient in healing a certain disease.
...
in most kinds of frequency bands, as illustrated in Figure~\ref{fig:1:United_States_Frequency_Allocations_Chart_2016_The_Radio_Spectrum} below, the entire RF spectrum is now allocated and free bands no longer exist, limiting the possibility of adding new usage.
%
...
Regulatory bodies in the world, like the Federal Communications Commission in the United States, as well as different independent measurement campaigns found that most radio frequency spectrum was inefficiently utilized
meaning that while a band can be allocated to a certain usage, it can be free from any user in certain time and/or place.
For instance, see \cite{valenta2010survey} for a survey on spectrum utilization in Europe.
%
...
These initiatives have focused cognitive-radio research on \textbf{Dynamic Spectrum Access (DSA)}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.65\textwidth]{United_States_Frequency_Allocations_Chart_2016_The_Radio_Spectrum.pdf}
    \caption{A chart representing the allocation of radio spectrum in the United States of (North) America, in $2016$. \copyright{} Wikimedia.}
...
We focus on one example, the \textbf{Opportunistic Spectrum Access} (OSA).

\TODOL{Je dois faire ça d'ici mercredi matin}

% FIXME how to explain OSA ?
...
\paragraph{Previous work in the SCEE team on OSA.}

expliquer ce que Christophe Moy, Wassim Jouini, Navik Modi etc ont fait depuis 2009


\paragraph{New challenges for the IoT networks.}
%
...
We answered partially to this problematic by the following research steps.

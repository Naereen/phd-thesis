...
\TODOL{Mardi : je dois encore relire le 1.1 pour les fautes d'orthographe (et les éventuels erreurs historiques) !}
\TODOL{Mardi : je dois encore terminer le 1.2 et être plus concis ?}
\TODOL{Je dois écrire la fin du 1.3 en max 2 pages : j'ai déjà le plan, je dois développer}
...
This model constitutes the first article written during this thesis, \cite{Bonnefoi17}, with R. Bonnefoi, another PhD student of our team SCEE.
...
In Section~\ref{sec:4:firstModel}, we propose the first PoC of using decentralized MAB learning in a simplified ``IoT'' like wireless network \cite{Besson2018ICT}.
...
We then consider extension of our first model, following the idea of packet retransmission that is at the root of the ALOHA protocol \cite{Abramson1970,Roberts75}.
...
We present the model in Section~\ref{sec:4:retransmissions}, along with mathematical justifications of the potential of using this second-stage MAB learning for retransmissions, and numerical illustrations \cite{Bonnefoi2019WCNC}.
...
%
In the two models presented before, the core concept is to let all the dynamic devices of a IoT network run a fully decentralized learning algorithm,
each of them targeting their local objective of maximizing their cumulated reward (in a selfish way),
in order to optimize the complete system.
...
\TODOL{Emilie me dit de continuer comme ça :
    En tout cas ce chapitre 1 commence à avoir bonne forme pour l'école
    doctorale, il n'y aura plus qu'à finaliser la deuxième partie de la
    section 1.3 (la première est très bien). Tu tiens le bon bout !
}
...
%
FIXME explain non stationarity !

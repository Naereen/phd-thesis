...
\TODOL{Mardi soir : je dois encore relire le 1.1 pour les éventuelles erreurs historiques !}
\TODOL{Mardi soir : je dois encore améliorer le 1.2 et être plus concis ?}
\TODOL{Mardi soir : je dois relire la fin du 1.3, et terminer le dernier point}
...
Research on MAB has produced a vast literature in the $2000$s \cite{Auer02,Auer02NonStochastic,Audibert2009minimax} and continues to be a topic of high interest, as illustrated by the surveys and books \cite{Bubeck12,LattimoreBanditAlgorithmsBook,Slivkins2019}.
...
\end{itemize}
\TODOL{FIXME more specificities? Est-ce que je dois mentionner d'autres contraintes ? Il me faudrait aussi une ou deux références genre survey des standards IoT}
...
We confirm that famous and not too sophisticated algorithms, such as \UCB{} \cite{Auer02}, Thompson sampling \cite{Thompson33} and \klUCB{} \cite{KLUCBJournal}, are the most efficient in terms of regret, and offer a good balance between regret and (time and memory) complexity.
...
This work on aggregating MAB algorithms was motivated for the OSA case of Cognitive Radio, where many previous research works only considered the $\UCB_1$ algorithm without justifying this choice rigorously,
...
%
On the other hand, the second model we study considers packet retransmissions, and while the concept stays similar, the application of an efficient decentralized learning results in both an increased battery life for each of the devices, as well as an increase QoS for the complete network.
...
%
Even without changing anything on the level of IoT standards, our proposal is just an add-on capability that can be used on a unit-per-unit basis.
...
The difficulty thus resides in the fact that we are trying to analyze MAB algorithms, designed to tackle stationary problems, on a non-stationary problem. %XXX
...
If we focus on one IoT device, we can assume that the environment (\ie, the surrounding devices) is stationary on some time intervals, which leads to the last contribution and chapter of this thesis.
...
\TODOL{Mardi soir je peux mieux expliquer, là je suis long et chiant je trouve}
...
%
The Appendix of this manuscript starts with a short overview of another contribution, in Appendix~\ref{app:2:DoublingTricks} we discuss about our work on doubling-trick

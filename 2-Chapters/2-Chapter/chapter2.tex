...
To analyze the algorithm $\UCB_1$, we denote again $k^*$ an optimal arm, and $C(t)$ is the arm chosen by the algorithm in round $t$.
...
If $\pi_k(t)$ denotes this posterior distribution, Thompson sampling makes its decision at time $t$ by first sampling a random sample $s_k \sim \pi_k(t)$, and then playing according to the ``optimism under uncertainty'' philosophy if these samples represent the arms (expected) quality: $C(t) \in \argmax_k s_k$.

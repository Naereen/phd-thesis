...
%
We then expose two simple strategies, which performs efficiently only if they are tuned using a prior knowledge of the problem difficulty, but are thus unusable for practical applications.
%
...
%
Like for the lower-bound, we have $R_T \leq (\max_{k: \Delta_k > 0} \Delta_k) T_0 + p (T - T_0)$, thus $R_T \leq K T_0 + K \exp(-T_0 d^2/4) T$ using Hoeffding's inequality again. Optimizing on $T_0$ gives $T_0 = 4/d^2 \log(d^2 T / 4)$
...
%
This strategy is also referred to as ``explore-first'', it can also achieve $R_T = \bigO{T^{2/3} (K \log(T))^{1/3}}$ without prior knowledge on the problem, as shown in Section~1 of \cite{Slivkins2019}.
...
The indexes should represent the expected quality of each arm, thus index policies play the arm that maximizes their index, \ie, $C(t) = \argmax_k U_k(t)$.
If more than one index maximizes $(U_k(t))_k$, the arm is chosen among the set, usually in a uniform random manner: $C(t) \sim \cU(\argmax_k U_k(t))$.
%
...
The \UCB{} indexes are computed as follows:
\begin{align}\label{eq:UCB_index}
    \forall k,\;\;\;
    U_k^{\UCB}(t) = \sup\limits_{q \in [a, b]} \left\{ q : \mathrm{kl}(\frac{X_k(t)}{N_k(t)}, q) \leq \frac{c \log(t)}{N_k(t)} \right\}.
\end{align}
...
\begin{align}\label{eq:klUCB_index}
    \forall k,\;\;\;
    U_k^{\klUCB}(t) = \sup\limits_{q \in [a, b]} \left\{ q : \mathrm{kl}(\frac{X_k(t)}{N_k(t)}, q) \leq \frac{c \log(t)}{N_k(t)} \right\}.
\end{align}
...
Many variants have been proposed in the multi-armed bandits literature.
For instance, \UCB-H (and \klUCB-H) replaces the $\log(t)$ by a $\log(T)$, and it corresponds to the first algorithm analyzed in \cite{Auer02}.
Using an estimator for the variance leads to \UCB-V, efficient for instance for problems where the arms are Gaussian distributions with unknown variance.
Other variants include \UCB$^+$, \UCB-dagger from \cite{Lattimore2018refining}, {\UCB}oost, and many more.
%
They are usually comparable to the original \UCB{} algorithm, in terms of time and memory complexity, and achieves improved regret upper bounds in the same setting or extension of the initial setting.


\paragraph{Other policies inspired by \UCB}
%
The ``Successive Elimination'' policy also uses confidence bounds, and use both UCB and Lower Confidence Bounds (LCB). After playing each active arms once, it eliminates arm $a$ if it is statistically safe that it is sub-optimal, \ie, if there is another arm $a' \neq a$ such that $\UCB_a(t) < \mathrm{LCB}_{a'}(t)$.
...
If $\pi_k(t)$ denotes this posterior distribution, Thompson sampling makes its decision at time $t$ by first sampling a random sample $s_k \sim \pi_k(t)$, and then playing according to the ``optimism in face of uncertainty'' philosophy if these samples represent the arms (expected) quality: $C(t) \in \argmax_k s_k$.
...
and maintain a distribution $\mathrm{Beta}(X_k(t)+1,N_k(t)-X_k(t)+1)$ after $X_k(t)$ successes observed on arm $k$ and $N_k(t)-X_k(t)$.
...
Thompson sampling was proved to achieve asymptotical optimal problem-dependent bounds \cite{Kaufmann12Thompson,AgrawalGoyal11}.
...
Finally, we would like to mention that a recent work \cite{KimTewari2019} presented an approach that closes the gap between the principle of optimism in the face of uncertainty and the Bayesian point-of-view.
...
We briefly mention other families of policies.
...
It is a recent variant of the $\mathrm{Exp3}$ algorithm
is $\mathrm{Exp}3^{++}$ from \cite{Seldin17},
and it uses an adaptive tuning of its parameters.
...
\begin{align}\label{eq:2:indexes_for_MOSS_klUCBpp}
    ...
    \forall k,\;\;\;
    U^{\KLUCBpp}_k(t) &= \sup\limits_{q \in [a, b]} \left\{ q : \mathrm{kl}(\hat{\mu}_k(t), q) \leq \frac{c g(N_k(t), T, K)}{N_k(t)} \right\}.
\end{align}
%
...
they enjoy regret bounds like the following, for two constants $c_{\cA}$ and $c'_{\cA}$ that depend on the algorithm, and a constant $C_I$ that depend on the problem instance only:
\begin{equation}\label{eq:2:regretBoundsForHybridPolicies}
    R_T^{\cA} \leq \min( c_{\cA} \sqrt{K T}, c'_{\cA} C_I \log(T)).
\end{equation}
...
To detail this, first let $f(T, K) = \lfloor \rfloor$ for $\gamma=1/5$,
...
\begin{align}\label{eq:2:indexes_for_klUCBswitch}
    ...
    U^{\mathrm{MOSS}+}_k(t) &= \frac{X_k(t)}{N_k(t)} + \sqrt{\max\left(0, \frac{\log\left(\frac{T}{K N_k(t)}\right)}{N_k(t)}\right)}. \\
    \forall k,\;\;\;
    U^{\KLUCB+}_k(t) &= \sup\limits_{q \in [a, b]} \left\{ q : \mathrm{kl}(\hat{\mu}_k(t), q) \leq \frac{c \log(T / (K * N_k(t)))}{N_k(t)} \right\} \\
    \forall k,\;\;\;

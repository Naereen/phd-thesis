...
When the player decides at time $t$ to play (or pull) the arm $A(t)\in[K]$,
where this notation denotes $[K] \eqdef \{1,\dots,K\}$, she receives a reward $r(t) \eqdef Y_{A(t),t}\in\R$, and so on for a finite number of steps $t$ (or rounds), from $t=1$ until $t=T$ for an \emph{horizon} $T$.
%

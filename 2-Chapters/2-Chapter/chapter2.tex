...
When the player decides at time $t$ to play (or pull) the arm $A(t)\in\{1,\dots,K\}=[K]$, she receives a reward $r_{A(t)}(t)\in\R$, and so on for a finite number of steps $t$, from $t=1$ until $t=T$ for an \emph{horizon} $T$.
%
...
\begin{figure}[h!]
    ...
    \resizebox{0.50\textwidth}{!}{
        \begin{tikzpicture}[auto, node distance=5cm, >=latex]
            ...
            \draw [->] (environment) to[bend left=90] node[pos=0.5] {Observes a \textbf{real-valued} reward $r_k(t) \in \R$} (player);
            %
        \end{tikzpicture}
    }
...
Then this quantity also depends on the player's decision-making policy, \ie, the choices $A(t)$, that are based on (all) the \emph{past} observations, \ie, $A(1)$, $r_{A(1)}(1)$, $\dots$, $A(t-1)$, $r_{A(t-1)}(t-1)$, and possibly on an external source of randomness.
%
...
\begin{figure}[h!]  % [htbp]
    ...
    \caption{Screenshot of the demonstration, at the end of the game after $T=100$ steps, where the player suffers from an empirical regret of $R^{\UCB_1}_T = \mu^* T - 56 = 0.7 T - 56 = 14$ after following the $\UCB_1$ algorithm.
...
\textbf{We also focus only on real-valued rewards}, meaning that $r_k(t)\in\R$ for all arm $k$ and time $t$.
...
In all this thesis we restrict to distributions that have a certain form, and \textbf{we focus either on real-valued distributions lying in an exponential families or bounded distributions}.
...
%
\begin{itemize}\label{par:2:notationsExponentialFamiliesBernoulliGaussian}
    \item
...
\begin{proof}\label{proof:2:UCBregretBound}
    \TODOL{J'aimerai bien rajouter des couleurs sur $k$ et $k^*$ partout plus bas ?}
...
of the average rewards (\ie, $\frac{1}{t} \sum\limits_{s=1}^t r_{A(s)}(s)$), which should converge to $\mu^*$ when $t\to\infty$ for efficient algorithms.
%
...
\begin{figure}[h!]  % [htbp]
	% \centering
	\includegraphics[width=1.20\linewidth]{SP__K5_T10000_N1000__7_algos/main____env1-1_2506036032481767447.pdf}
...
We prove it below using simple real analysis arguments\footnote{~The proof is inspired by Lemma~10.2 from \cite{LattimoreBanditAlgorithmsBook} (version $1699$ of January $2019$).},
%

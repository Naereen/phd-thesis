...
%
As explained in Chapter~\ref{chapter:1}, we focus on decision-making models with a finite number of resources, called arms, and on stochastic models, where an arm is associated to a binary or real-valued distribution.
...
We define the notion of regret of an algorithm, and present important results from the literature.
A short review of the most important families of MAB algorithms is then given,
and we highlight some algorithms that are crucially used in the rest of this thesis, like \UCB, \klUCB{} and Thompson sampling.
%
...
Except the naive and simple strategies given as introductory examples, most of the policies presented above are order-optimal for at least one of the model of Bernoulli distributed problems, bounded rewards or one-dimensional exponential families,
...
This also serves as a first example of simulation performed with our library SMPyBandits, before the large-scale experiments of Section~\ref{sec:3:reviewSPAlgorithms} below.
%
In the rest of this thesis, we mainly use $\UCB_1$ or Thompson sampling when simplicity and realistic deployment is favored (in Chapter~\ref{chapter:4}), and \klUCB{} when we give mathematical developments (in Chapters~\ref{chapter:5} and \ref{chapter:6}).
...
for the two problems of stationary multi-players and piece-wise stationary bandits.

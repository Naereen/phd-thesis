...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{2-Chapters/2-Chapter/Images/example_of_a_5_arm_bandit_problem.png}
...
\begin{equation}\label{eq:2:exponentialFamily}
    \Pr(x | \lambda) = h(x) \exp \left( \eta^T T(x) - A(\eta) \right),
\end{equation}
...
\begin{itemize}
    \item
    \textbf{Bernoulli distributions} can be seen as an exponential family with
    $T(x) = x$ and $h(x) = 1$,
    ...
    \item
    \textbf{Gaussian distributions} use
...
%
As introduced above, let $N_k(t) = \sum_{s=1}^t \mathbbm{1}(A(s) = k)$ be the number of times arm $k$ was selected between times $1$ and $t$.
...
We remind in this section two results about bandit algorithms \emph{cannot} do:
first a worst case result that states that any algorithm can perform as badly as $\Omega(\sqrt(K T))$ on a certain problem instance (a result from \cite{Auer02NonStochastic}),
then a problem-dependent lower-bound that states that any algorithm suffers a regret at least $\Omega(\log T)$ on any problem (from \cite{LaiRobbins85}).
...
For more details, see Chapter~2 in \cite{Slivkins2019}.
...
First, let $\cI$ denote the set of all problem instances, with hypotheses that there are $K \geq 2$ arms whose rewards lie in $[0,1]$.

\begin{theorem}\label{thm:2:firstLogTLowerBound}
    No algorithm $\cA$ can achieve a regret $R^{\cA}(T) = \smallO{c_I \log(T)}$ for all problem instances $I \in \cI$,
...
Then we consider algorithms that uniformly efficient, meaning that we rule out algorithms that can achieve low regret on some problem instance while achieving linear regret on other instances.
...
\begin{defn}[Uniformly efficient algorithm]\label{def:2:uniformlyEfficientAlgorithm}
    An algorithm $\cA$ is \emph{uniformly efficient} if it satisfies
    \begin{equation}
        R^{\cA}(T) \leq \bigO{C_{I,\alpha} T^{\alpha}}
    \end{equation}
...
In this thesis, we only study analytically the mean regret $R^{\cA}$ (in Chapters \ref{chapter:5,chapter:6}), but we quickly mention other measures of performances introduced in the literature.
...
The switching cost $SC^{\cA}(T)$ counts how many times the player's decision has changed from one time step to the next one, \ie, $SC^{\cA}(T) = \sum_{t=1}^{T-1} \indic(A(t) \neq A(t+1))$.

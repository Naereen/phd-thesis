...
We give an overview of the main families of algorithms designed to tackle stochastic MAB problems, and we mainly focus on the $\UCB_1$, \klUCB{} and Thompson sampling algorithms, as they are used in the rest of the manuscript.
...
\textbf{Hypotheses in this thesis.}
%
...
\begin{lemma}[Regret decomposition]\label{lem:2:RegretDecomposition}
    ...
    weighted by their gaps, like this:
...
This family is non-empty as it contains for instance the $\UCB_1$ algorithm, as its regret is proven to be logarithmic on any problem instance.
...
\begin{theorem}\label{thm:2:forSecondLogTLowerBound}
    ...
    \begin{itemize}
        ...
        $C_I = \mu^* (1 - \mu^*) \sum\limits_{\substack{k=1,\dots,K\\\Delta_k > 0}} \frac{1}{\Delta_k}$.
        % \end{equation}
        \item
        ...
        $C_I = \sum\limits_{\substack{k=1,\dots,K\\\Delta_k > 0}} \frac{\Delta_k}{\KL(\mu_k, \mu^*)} - \varepsilon$.
        % \end{equation}
    \end{itemize}
\end{theorem}
...
% - Fairness ?
Finally, another interesting measure of performance is the \emph{fairness}.
It was not studied much for single-player problems, and usually it means that each arm must be explored at least a given fraction of the total number of rounds $T$ (\ie, ``arm-wise fairness'').
...
%
To prove a bound on $p$, we use the following lemma, well known as Hoeffding's inequality, from \cite{hoeffding1963probability}.

\begin{lemma}[Hoeffding's inequality]\label{lem:2:HoeffdingInequality}
...
\begin{theorem}[Regret bound for $\UCB_1$]\label{thm:2:UCBregretBound}
    ...
    of means $\mu_1,\dots,\mu_k$, and any horizon $T>10$,
    the $\UCB_1$ algorithm verifies the following asymptotic regret bound
    \begin{equation}\label{eq:2:UCBregretBound}
        R_T^{\cA}(I) \leq \sum_{k: \Delta_k>0} \frac{8}{\Delta_k} \log(T) + \smallO{\log(T)}.
    \end{equation}
\end{theorem}
%
\begin{proof}
    For simplicity, we focus on bounded distributions in $[0,1]$, for which Hoeffding's inequality gives \eqref{eq:2:HoeffdingInequality} as stated above in Lemma~\ref\label{lem:2:HoeffdingInequality}.
    To analyze the algorithm $\UCB_1$, we restrict to $\alpha=2$ and we proceed as follows:

    $i)$ First, for any arm $k$, Hoeffding's inequality shows that $\Pr(\UCB_k(t) \leq \mu_k) \leq \bigO{1/t^{4}}$ independently of $N_k(t)$,
    and so $\sum_{t=1}^T \Pr(\UCB_k(t) \leq \mu_k) = \smallO{\log(T)}$: the \UCB{} are almost always above the empirical means (\ie, they are true \emph{upper} confidence bounds).

    $ii)$
    Then, we denote again $k^*$ an optimal arm, and $A(t)$ is the arm chosen in round $t$.
    According to the algorithm, the probability that $k$ is chosen is upper-bounded by $\Pr(\UCB_k(t) \leq \UCB_{k^*}(t))$.
    This can happen if $\UCB_{k^*}(t) \leq \mu_{k^*}$, which from $i)$ is very unlikely, or if $\UCB_k(t) \leq \UCB_{k^*}(t) > \mu_{k^*}$.
    By writing $\mu_k = \mu_{k^*} - \Delta_k$, and because $\UCB_k(t)$ are using $N_k(t)$ \iid{} samples from arm $k$, of mean $\mu_k$, Hoeffding's inequality also shows that $\Pr(\UCB_k(t) - \mu_k > \Delta_k) \leq 8/(\Delta_k^2 t)$ (the confidence radius $\xi_k(t)$ is actually built exactly to obtain this bound).
    %
    And thus\footnote{~We used the partial sum of the harmonic series: $\sum\limits_{t=1}^T \frac{1}{t} \underset{T\to\infty}{\sim} \log(T)$.} $\sum_{t=1}^T \Pr(\UCB_k(t) - \mu_k > \Delta_k) \leq 1/\Delta_k^2 \sum_{t=1}^T 8/t = 1/\Delta_k^2 8\log(T)$.

    To sum up, we obtain the following bound on $N_k(T)$ for any sub-optimal arm $k$:
    \begin{align*}
        \E[N_k(T)] = \sum_{t=1}^T \Pr(\ind(A(t)=k)) &\leq \sum_{t=1}^T \Pr(\UCB_k(t) > \UCB_{k^*}(t)) \\
        &\leq \sum_{t=1}^T \Pr(\UCB_{k^*}(t) < \mu_{k^*}) + \sum_{t=1}^T \Pr(\UCB_{k^*}(t) < \mu_{k^*}) \\
        &\leq \smallO{\log(T)} + \frac{8}{\Delta_k^2} \log(T).
    \end{align*}
    %
    By using the regret decomposition from Lemma~\ref{lem:2:RegretDecomposition}, this gives $R_T^{\UCB_1} \leq \sum_{k: \Delta_k>0} \frac{8}{\Delta_k} \log(T) + \smallO{\log(T)}$.
    We preferred to use the $\smallO{\dots}$ Landau notation, instead of keeping the exact constants, for simplicity and clarity.
    More details on the proofs can be found for instance in Chapter~2 of \cite{Bubeck12}, or Section~1.3 of \cite{Slivkins2019}.
\end{proof}

What this theorem shows is that the $\UCB_1$ algorithm achieves order-optimal problem-dependent bounds with a $\cO(K\log(T)/\Delta^2)$ regret.
The $\UCB_1$ algorithm has the advantage of not relying on any prior knowledge of the problem difficulty, of being anytime, and computationally simple, for both memory and time aspects.

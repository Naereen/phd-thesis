...
%
\begin{proof}
    As we focus on bounded distributions in $[0,1]$, for which Hoeffding's inequality gives \eqref{eq:2:HoeffdingInequality}, as stated above in Lemma~\ref\label{lem:2:HoeffdingInequality}.
    ...
    \begin{equation}\label{eq:2:trickInUCBProof}
        ...
        \leq n + \sum_{t=n}^T \indic(A(t) = k, N_k(t) \geq n).
    \end{equation}
    ...
    $\widehat{\mu_{a,n}}(t)$ and $\xi_{a,n}(t)$ where $N_k(t)$ is replaced with $n$ if $n \leq N_k(t)$,
    \ie, $\widehat{\mu_{a,n}}(t) = \frac{1}{n} \sum_{s=1}^{t-1} r_k(s) \indic(A(s)=k, N_k(s) \leq n)$ and $\xi_{a,n}(t) = \sqrt{2\log(t) / n}$.
    %
    ...
    \begin{align}\label{eq:2:decompositionInUCBProof}
        ...
        & \leq n + \sum_{t=n}^T \indic(\widehat{\mu_{k^*,N_{k^*}(t)}}(t) + \xi_{k^*,N_{k^*}(t)}(t) < \widehat{\mu_{k,N_k(t)}}(t) + \xi_{k,N_k(t)}(t), N_k(t) \geq n)  \nonumber \\
        & \leq n + \sum_{t=n}^T \indic(\min_{0 < n_{k^*} < t} \widehat{\mu_{k^*,n_{k^*}}}(t) + \xi_{k^*,n_{k^*}}(t) < \max_{n < n_k < t} \widehat{\mu_{k,n_k}}(t) + \xi_{k,n_k}(t)) \nonumber \\
        \intertext{So two union bounds on $n_{k^*}$ and $n_k$ give}
        & \leq n + \sum_{t=n}^T \sum_{n_{k^*}=1}^t \sum_{n_{k}=n}^t  \indic( \widehat{\mu_{k^*,n_{k^*}}}(t) + \xi_{k^*,n_{k^*}}(t) < \widehat{\mu_{k,n_k}}(t) + \xi_{k,n_k}(t) )
    \end{align}

    Now, at time $t$, this event $\widehat{\mu_{k^*,n_{k^*}}}(t) + \xi_{k^*,n_{k^*}}(t) < \widehat{\mu_{k,n_k}}(t) + \xi_{k,n_k}(t)$
    implies at least one of the following three events:
    \begin{align*}
        I_1: & \widehat{\mu_{k^*,n_{k^*}}}(t) \leq \mu_{k^*} - \xi_{k^*,n_{k^*}}(t),
        I_2: & \widehat{\mu_{k,n_k}}(t) \geq \mu_k + \xi_{k,n_k}(t),
        I_3: & \mu_{k^*} < \mu_k + 2 \xi_{k,n_k}(t).
    \end{align*}

    For the first two events $I_1$ and $I_2$, we carefully fixed the number of samples so they are no longer random (they are respectively to $n_{k^*}$ and $n_k$), we can use Hoeffding's inequality to obtain $\Pr(I_j) \leq \frac{1}{t^{4}}$ for $j=1,2$.
    ...
    Thus if $n_k \geq \lceil \frac{8 \log(T)}{\Delta_k^2} \rceil \geq \lceil \frac{8 \log(t)}{\Delta_k^2} \rceil$,
    ...
    Thus we can take the expectation of $N_k(T)$ from equation~\eqref{eq:2:decompositionInUCBProof}, and continue by setting
    with $n = n_T = \lceil \frac{8 \log(T)}{\Delta_k^2} \rceil$:

    \begin{align}\label{eq:2:decompositionInUCBProof2}
        \E[ N_k(T) ]
        & \leq 1 + \frac{8 \log(T)}{\Delta_k^2} + \sum_{t=n_T}^T \sum_{n_{k^*}=1}^t \sum_{n_{k}=n_T}^t  \Pr( \widehat{\mu_{k^*,n_{k^*}}}(t) + \xi_{k^*,n_{k^*}}(t) < \widehat{\mu_{k,n_k}}(t) + \xi_{k,n_k}(t) ) \nonumber \\
        & \leq 1 + \frac{8 \log(T)}{\Delta_k^2} + \sum_{t=n_T}^T \sum_{n_{k^*}=1}^t \sum_{n_{k}=n_T}^t  \Pr( I_1 ) + \Pr( I_2 ) + \Pr( I_3 ) \nonumber \\
        & \leq 1 + \frac{8 \log(T)}{\Delta_k^2} + \sum_{t=n_T}^T \sum_{n_{k^*}=1}^t \sum_{n_{k}=n_T}^t  2 \frac{1}{t^4} \nonumber \\
        \intertext{We actually have that $\sum_{n_{k^*}=1}^t \sum_{n_{k}=n_T}^t  2 \frac{1}{t^4} \leq 2 \sum_{n_{k^*}=1}^t \sum_{n_{k}=1}^t \frac{1}{t^4} \leq 2 \sum_{n_{k^*}=1}^t \frac{1}{t^3} \leq 2 \frac{1}{t^2}$}
        & \leq \frac{8 \log(T)}{\Delta_k^2} + 1 + 2 \sum_{t=1}^T \frac{1}{t^2} \nonumber \\
        & \leq \frac{8 \log(T)}{\Delta_k^2} + 1 + 2 \sum_{t=1}^{\infty} \frac{1}{t^2} \nonumber \\
        \E[ N_k(T) ]
        & \leq \frac{8 \log(T)}{\Delta_k^2} + 1 + 2 \frac{\pi^2}{6}.
    \end{align}
    ...
    \begin{align*}
        R_T^{\UCB_1} &\leq \sum_{k: \Delta_k>0} \Delta_k \E[N_k(T)] \\
        & \leq \frac{8}{\Delta_k} \log(T) + \sum_{k: \Delta_k>0} \Delta_k \left( 1 + \frac{\pi^2}{3} \right)
        = \frac{8}{\Delta_k} \log(T) + \smallO{\log(T)}.
    \end{align*}
...
It requires a constant storage capacity for each arm (storing $X_k(t)$ and $N_k(t)$ require two integers or a \texttt{float} and an integer, \ie, $\bigO{1}$), giving a storage capacity of $\bigO{K}$, independently of the horizon $T$.

...
then it was shown in \cite{Auer02} that the regret is of the order of $K \log(T) / d + \smallO{T}$.


% https://smpybandits.github.io/docs/Policies.ExploreThenCommit.html
On the other hand, the ``explore-then-commit'' strategy, also introduced in \cite{SuttonBarto2018} and studied for instance in \cite{GarivierETC2016}, consists in first exploring uniformly the $K$ arms for $T_0$ time steps, then only exploiting the best arm.
If $T_0$ is fixed independently of $T$ and the problem difficulty, the regret is again growing linearly.


Both strategies give linear regret if they are tuned independently of the problem at hand, but they can be tuned to be efficient (\ie, with logarithmic regret) if lower-bound on the gaps $\Delta_k$ is known.

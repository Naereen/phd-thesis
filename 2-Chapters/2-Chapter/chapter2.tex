...
\textbf{Consequences of Lemma~\ref{lem:2:RegretDecomposition}.}
%
...
The \textbf{switching cost} $SC^{\cA}(T)$ counts how many times the player's decision has changed from one time step to the next one, \ie, $SC^{\cA}(T) = \sum_{t=1}^{T-1} \indic(A(t) \neq A(t+1))$.

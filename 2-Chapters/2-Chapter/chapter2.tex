...
% ---------------------------------------------------
\subsubsection{Simple efficient strategies: $\varepsilon$-greedy and Explore-then-Exploit}
...
%
This strategy is also referred to as ``explore-first'', and in the case without prior knowledge on the problem, it can achieve $R_T = \bigO{T^{2/3} (K \log(T))^{1/3}}$ as shown in Section~1 of \cite{Slivkins2019}.
...
If $P_k(t)$ denotes this posterior distribution, Thompson sampling makes its decision at time $t$ by first sampling a random sample $s_k \sim P_k(t)$, and then playing according to the ``optimism in face of uncertainty'' philosophy if these samples represent the arms (expected) quality: $C(t) \argmax_k s_k$.
...
Finally, we would like to mention that a recent work \cite{KimTewari2019} presented an approach that lies between the principle of optimism in the face of uncertainty and the Bayesian point-of-view.
...
They showed that their Randomized Confidence Bound (RCB) algorithm can achieve the same regret upper bounds as \UCB{} or Thompson sampling, with uniform perturbations.
...
They are both index-based, using XXX

More recently, the \klUCB-switch from \cite{Garivier18} XXX

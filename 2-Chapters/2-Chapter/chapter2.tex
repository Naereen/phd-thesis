...
\begin{small}
    \begin{flushright}
        \emph{-- C'est cool ça, des \emph{bandits} haha !}\\
        Le Barbare,
        John Lang, \href{http://www.penofchaos.com/warham/donjon.htm}{\emph{Le Donjon de Naheulbeuk}},\\
...
%
For more details and a more formal introduction to multi-armed bandits, we suggest the interested reader to work on a very recent text-book by Slivkins \cite{Slivkins2019}.
...
%
We focus on the \textbf{stochastic} MAB, in which the reward stream is drawn from some (unknown) distributions, before the beginning of the game.
%
...
%
% We also restrict to \emph{stochastic} rewards, that is
...
The web-page looks like Figure~\ref{fig:2:example_of_a_5_arm_bandit_problem} below.
...
The initial information is reduced to $O_1 = (U_0)$, and the first decision is $A(1) = \cA_1(O_1)$. Usually, most algorithms start by selecting $A(1)=1,\dots,A(K)=K$ (or a permutation of the $K$ arms) in the $K$ first steps $t=1,\dots,K$ (\eg, Algorithm~\ref{algo:2:indexPolicy}).
...
In the literature, parametric assumptions on the rewards are sometimes considered, typically the assumption is that rewards belong to a real-valued distribution lying in an exponential family.
...
% We follow the notations from the course on statistical learning (Stat 260) taught in 2010 by Michael Jordan at the University of Berkeley \cite{JordanCourseStatBerkeley}, that are also the notations used for instance in Émilie Kaufmann's PhD in 2014 \cite{Kaufmann12PhD}.
...
and usually the literature focuses on regular and minimal exponential families (\ie, when the natural parameter space is a non-empty open set in $\R$).

We mention two important results on exponential families:

\begin{enumerate}
    \item
    \emph{A distribution in such a family is entirely characterized by its parameter $\eta$}.
...
They are not studied in this thesis, even though we did implement them in SMPyBandits.
...
In the model of Opportunistic Spectrum Access (OSA) with sensing \cite{Jouini09,Jouini10}, the samples $Y_{k,t}$ represent the feedback obtained by the CR-equipped device after sensing the channel $k$, at the beginning of its $t$-th transmission slot.
...
the optimal strategy for this bandit problem is to always pull one of the optimal arms (it can be not unique), but it is unrealistic as the player does not know the true means, nor the optimal arms (as long as they are not all optimal, \ie, in nontrivial problems).
%
...
%
Such a decomposition of the regret can be useful for at least two reasons:
...
and a significant part of the research on MAB algorithms has focused on finding algorithms that match the Lai and Robbins' lower-bound asymptotically \cite{LaiRobbins85}.
%
...
otherwise the algorithm is said to be \emph{order-optimal} if it matches the lower-bound with a larger constant.
...
Note that the lower-bound of Lai and Robbins, proven in \cite{LaiRobbins85}, is more general and applies to any one-dimensional exponential family $\cE$, by replacing the Bernoulli Kullback-Leibler divergence $\kl$ with the KL divergence of this family, $\kl_{\cE}$.
...
We give an example of such a result below.
...
we can visualize the entire \emph{distribution} as a histogram, or the variance of the values of $R_T^{\cA}$, and if the number $N$ is reasonably large, we can verify that the regret appears logarithmic for all runs.
...
%
It is relevant for cognitive radio applications, as a hardware reconfiguration costs energy, and so changing channel from two consecutive time steps has a nontrivial energy cost, as highlighted in \cite{modiDemo2016}.
...
%
The player \textcolor{darkgreen}{\emph{only explores}} without using the collected information, and this strategy fails dramatically for any nontrivial problem (\ie, linear regret).
Indeed it obtains a linear (mean) regret $R_T = \frac{1}{K} \sum_{k=1}^K \Delta_k T$
which gives $R_T \propto T$ for any problem with at least one sub-optimal arm (\ie, all nontrivial problems, ruling out the case where $\mu_1=\mu_2=\dots=\mu_K$).
...
% % The first is detailed in
% % Algorithm~\ref{algo:2:simpleStrategies}.
% Algorithms~\ref{algo:2:simpleStrategies_EpsilonGreedy} and \ref{algo:2:simpleStrategies_ExploreThenExploit} below.
% % Algorithm~\ref{algo:2:simpleStrategies_EpsilonGreedy}.
% % and \ref{algo:2:simpleStrategies_ExploreThenExploit}.
...
-- The \textcolor{deeppurple}{$\varepsilon$-greedy strategy} (in Algorithm~\ref{algo:2:simpleStrategies_EpsilonGreedy})
...
-- The ``\textcolor{deepgold}{Explore-then-Exploit}'' strategy (in Algorithm~\ref{algo:2:simpleStrategies_ExploreThenExploit}), presented for instance in \cite{Bubeck12}, first explores uniformly the $K$ arms for $T_0$ time steps, then only exploits the arm identified as the best arm after these first $T_0$ steps
...
\begin{leftbar}[lemmabar]  % XXX leftbar lemmabar, comment if needed
    ...
    \vspace*{-5pt}  % FIXME vspace negative to compensate the \end{leftbar}
    % \begin{small}
...
\paragraph{The $\UCB_1$ index policy: using Hoeffding's inequality to build confidence intervals.}
%
...
\begin{smallproof}\label{proof:2:UCBregretBound}
    ...
    % \TODOL{J'aimerai bien rajouter des couleurs sur $k$ et $k^*$ partout plus bas ? si c'est bien fait, ça peut aider la lecture, par exemple \textcolor{darkgreen}{$k$} et \textcolor{deepgold}{$k^*$}.}
...
We see in this Theorem~\ref{thm:2:UCBregretBound} that $\UCB_1$ has the advantage of not relying on any prior knowledge of the problem difficulty, of being anytime and computationally simple, for both memory and time aspects.
...
the \klUCB{} algorithm rather uses Chernoff's concentration inequality for the considered exponential family \cite{chernoff1952measure,chernoff1981note}.
...
The second one is less obvious: one has to solve the optimization problem in \eqref{eq:2:klUCB_index}.
...
but instead of maximizing the UCB, a random sample is simply taken on each interval $s_k \sim \cU(\left[ \mathrm{LCB}_k(t), \UCB_k(t) \right])$, and the played arm is the one maximizing this sample, \ie, $A(t) \in \argmax_k s_k$.
...
% - One of the first paper I read in 2016 was defining AdBandits, https://smpybandits.github.io/docs/Policies.AdBandits.html, a mixture between Thompson sampling and Bayes-UCB (very efficient in practice)
...
% which comes with an interactive online demonstration in order to better understand the behavior of the algorithm, hosted at \href{https://j2kun.github.io/mwua/index.html}{\texttt{j2kun.GitHub.io/mwua/}} (accessed the 24th of May 2019)
...
We consider $K=5$ arms following Bernoulli distributions, of means $\mu_1=0.6,\mu_2=0.2,\mu_3=0.55,\mu_4=0.7,\mu_5=0.5$ (unknown to the algorithms), and first a horizon $T=100$ (like in the online demo) then a larger value of $T=10000$.
...
%
We then include efficient algorithms, that do not need prior knowledge of the $T$ (they are anytime) nor $\Delta$ (they are robust and efficient on unknown problems):
$\UCB_1$ (with $\alpha=1$),
...
\begin{figure}[h!]  % [htbp]
	% \centering
    % \includegraphics[width=1.05\linewidth]{SP__K5_T10000_N1000__7_algos/main_MeanRewards____env1-1_2506036032481767447.pdf}
...
\begin{figure}[h!]  % [htbp]
    \centering
    % [][$7$ algorithms, including unefficient ones]
    \subfloat{
        % \includegraphics[width=0.84\linewidth]{SP__K5_T10000_N1000__7_algos/main____env1-1_2506036032481767447.pdf}
        \includegraphics[width=0.93\linewidth]{SP__K5_T10000_N1000__7_algos/main____env1-1_3198350886837790844.pdf}
        \label{fig:2:meanRegretAsFunctionOfTimeForDifferentAlgorithmsT10000N1000_7_algos}
    }
    \newline
    \vspace*{-20pt}%FIXME
    % [][$3$ algorithms, only the most efficient]
    \subfloat{
        \includegraphics[width=0.93\linewidth]{SP__K5_T10000_N1000__3_algos/main____env1-1_776571591054077529.pdf}
        \label{fig:2:meanRegretAsFunctionOfTimeForDifferentAlgorithmsT10000N1000_3_algos}
    }
	% \caption{Mean regret $R_t$ as function of $t$ for $T=10000$ and $N=1000$.}
	\caption{Mean regret $R_t$ as function of $t$ for $T=10000$ and $N=1000$. The 3 most efficient algorithms: $\UCB_1$, \klUCB{} and Thompson Sampling achieve logarithmic regret.}
...
%
% To clearly understand the difference in their empirical behaviors,
...
%  (see Figures~\ref{fig:2:example_of_a_5_arm_bandit_problem} and \ref{fig:2:example_of_a_5_arm_bandit_problem__step100}).
...
% the large-scale experiments of Section~\ref{sec:3:reviewSPAlgorithms} below.
%
...
% \begin{figure}[h!]  % [htbp]
% 	% \centering
% 	\caption{Mean regret $R_t$ as function of $t$ for $T=10000$ and $N=1000$, for the 3 most efficient algorithms: $\UCB_1$, \klUCB{} and Thompson Sampling.}
% 	\label{fig:2:meanRegretAsFunctionOfTimeForDifferentAlgorithmsT10000N1000_onlyLogarithic}
% \end{figure}
...
% We take another approach in Chapter~\ref{chapter:4}, where the
We generalize this MAB model in Chapter~\ref{chapter:4}, by studying decentralized learning of a large set of independent players, all having different random activation times.
...
% and we were unfortunately unable to obtain any strong theoretical results under such loose hypotheses.

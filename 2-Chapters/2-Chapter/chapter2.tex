...
\begin{proof}\label{proof:2:RegretDecomposition}
    The tower rule of expectation yields
    $\E \left[ \sum_{t=1}^T r_{A(t)}(t) \right] = \sum_{k=1}^K \mu_k \E \left[ N_k(T) \right]$ as above, and thus $R_T^{\cA} = \sum_{k=1}^K \Delta_k \E[ N_k(T) ]$.
...
Chose any small $\varepsilon$ such that $\varepsilon \ll \sqrt{4 \log(T) / T}$, then if the means are $\mu_1 = 1/2$ and $\mu_2 = 1/2 - \varepsilon$.
The constant $C_I$ from \eqref{eq:2:forSecondLogTLowerBound} is $C_I = 1 / (4 \varepsilon)$ and $\Delta=\varepsilon$,
and thus the logarithmic upper-bound is useless in such case, as
\begin{equation}
...
In this Section, we start by discussing two naive strategies which fail dramatically.
%
We then expose two simple strategies, which performs efficiently only if they are tuned using a prior knowledge of the problem difficulty, and are thus unusable for practical applications.
%
We continue by presenting a first efficient policy, \UCB, that uses upper confidence bounds (UCB) on the mean estimates, and is known to be order-optimal for bounded rewards.
Two other well-known and optimal algorithms are exposed: \klUCB{} extends the idea of \UCB{} but use smaller confidence intervals and thus typically has better theoretical results, and Thompson sampling which takes a Bayesian point-of-view instead of relying on the principle of optimism in the face of uncertainty.
%
Finally, we conclude this Section by briefly exposing other families of algorithms.


\textbf{Implementation:}
%
We describe our library SMPyBandits in more details in Chapter~\ref{chapter:3}, but all the described algorithms are implemented and documented in SMPyBandits, alongside with many more algorithms (there were about 65 for single-player stochastic problems at the time of writing this thesis).
A complete list of the implemented policies can be found on the following web page on the documentation,
\href{https://smpybandits.github.io/docs/Policies.html}{\texttt{SMPyBandits.GitHub.io/docs/Policies.html}}, and on the \texttt{Policies} module.


% ---------------------------------------------------
\subsubsection{Naive strategies: pure exploitation or pure exploration}

Let us first describe two naive strategies, that both fail dramatically.
We recall the notations introduced above, the sum of rewards $X_k(t) = \sum_{s=1}^t r_k(s) \mathbbm{1}(A(s) = k)$, and the number of samples $N_k(t) = \sum_{s=1}^t \mathbbm{1}(A(s) = k)$.
%
...
On the first hand, consider the ``\emph{Follow-the-Leader}'' strategy which consists in first playing once each arm, then always playing $A(t)\in\argmax \widehat{\mu_k}(t)$.
...
Indeed consider $K=2$ Bernoulli arms of means $\mu_1=1/2$ and $\mu_2=\varepsilon$ where $\varepsilon < 1/2$, then with probability $\varepsilon/2$ the player observes a reward of $0$ for arm $1$ then $1$ for arm $2$ on the first rounds, and so she will play arm $2$ for all the $T-2$ rounds, giving a linear regret $R_T \geq \frac{\varepsilon}{2}\left(1/2 - \varepsilon\right) (T-1)$.

On the other hand, consider the uniform strategy which consists in always playing the $K$ uniformly at random.
...
Indeed it obtains a linear regret $R_T = \frac{1}{K} \sum_{k=1}^K \Delta_k T$
...
% ---------------------------------------------------
\subsubsection{Simple efficient strategies: $\varepsilon$-greedy and Explore-then-Commit}

As the two examples illustrated, an efficient strategy needs to solve the trade-off between exploration and exploitation.
The two following solutions both consist in splitting the $T$ time steps into $T_0$ steps of exploration and $T-T_0$ steps of exploitation, either in an alternative way or in a fixed way (``explore then exploit'').

% https://smpybandits.github.io/docs/Policies.EpsilonGreedy.html
On the first hand, the $\varepsilon$-greedy strategy, from \cite{SuttonBarto2018}, consists in alternating exploration and exploitation at a certain ratio.
...
But if we consider $\varepsilon_t$ decreasing with time $t$, for instance $\varepsilon_t = 6 K / (d^2 t)$, for a constant $0 < d < \min_{k: \Delta_k > 0} \Delta_k$,
...
On the other hand, the ``explore-then-exploit'' strategy, also introduced in \cite{SuttonBarto2018} and studied for instance in \cite{GarivierETC2016}, consists in first exploring uniformly the $K$ arms for $T_0$ time steps, then only exploiting the best arm.
If $T_0$ is fixed independently of $T$ and the problem difficulty, the regret is again growing linearly, as $R_T \geq (\min_{k: \Delta_k > 0} \Delta_k) K T_0 + p (T - T_0)$, if $p$ denotes the probability that the chosen arm is not optimal.
As $p$ only depends on the number of collected samples $T_0$ and not $T$, $R_T = \Omega(T)$.

\begin{proof}
    For the case of two arms,
    for instance, if the distributions are Bernoulli (or sub-Gaussian), we can use Hoeffding's inequality from \cite{hoeffding1963probability} to control this probability $p$.
    %
    Indeed if $Z_1,\dots,Z_n$ are \iid{} samples from a Bernoulli distribution of parameter $\mu$, of mean $\hat{Z}_n$ we have
    \begin{align}
        \forall x < \mu, \bP(\hat{Z}_n) \leq \exp(-2 n (x-\mu)^2),\\
        \forall x > \mu, \bP(\hat{Z}_n) \geq \exp(-2 n (x-\mu)^2).
    \end{align}
    %
    If $\mu_1 = \mu_2 + \Delta$, then
    $p = \bP(\widehat{\mu_1} < \widehat{\mu_2}) \leq \bP(\widehat{\mu_1} < \widehat{\mu_2}) + \bP(\widehat{\mu_1} < \widehat{\mu_2})$, and both terms can be bounded by using Hoeffding's inequality with $n=T_0/2$ samples,
    ...
    %
    It shows that $p$ is a constant wrt $T$.
\end{proof}

Using the same analysis, if a lower-bound $d$ on the positive gaps is known beforehand, one can find a tuning of $T_0$ that gives a logarithmic regret, like for the previous strategy.
%
Like for the lower-bound, we have $R_T \leq (\max_{k: \Delta_k > 0} \Delta_k) K T_0 + p (T - T_0)$, thus $R_T \leq K T_0 + K \exp(-T_0 d^2/4) T$ using Hoeffding's inequality again. Optimizing on $T_0$ gives $T_0 = 4/d^2 \log(d^2 T / 4)$
and $R_T \leq K 4/d^2 (1 + \log(d^2 T / 4)) = \bigO{\log(T)}$.


An extension of the ``exploit-then-exploit'' is to consider not a fixed time $T_0$ but a random time $\tau$ at which exploration stops.
This time $\tau$ must be a \emph{stopping time} in the sense that it is a measurable random variable, dependent of the past observations, as introduced by \cite{Wald45}. The strategy is then referred to as ``explore-then-commit'' (ETC), and the idea is to use a statistical test at every time step $t$, and stop as soon as enough samples were collected to effectively identify the best arm with a certain confidence level $\delta$. Choosing $\delta \propto 1/T$ typically lead to a logarithmic upper-bound on the regret.


For both cases, the strategy obtains a linear regret if it is tuned independently of the problem at hand, but they can be tuned to be efficient (\ie, with logarithmic regret) if lower-bound on the gaps $\Delta_k$ is known.
%  \ie, they require a prior knowledge on the problem difficulty.
As such, they are not applicable on an unknown problem and are less interesting from a practical point-of-view.
...
\TODOL{Expliquer UCB}
\TODOL{Expliquer que les indices UCB = des UCB sur les moyennes + principle of optimism in face of uncertainty}
...
\begin{figure}[h!]
	...
	\begin{algorithm}[H]
		...
		\caption{A generic index policy.}
...
compute the index $U_k(t) = \widehat{\mu_k}(t) + \sqrt{\alpha \log(t) / N_k(t)}$.

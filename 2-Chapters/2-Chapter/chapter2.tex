...
\TODOL{Intégrer les derniers commentaires d'Emilie, entre la fin des détails sur les familles exponentielles (avec les deux exemples), et quand je commence en Section~\ref{sub:2:otherMeasuresPerformance} à parler des autres mesures !}
...
We also index the Landau notations $o(\bullet)$ and $\bigO{\bullet}$ by $I$,
for instance $R_T^{\cA}(I) = \smallO_I{\log(T)}$ means $R_T^{\cA}(I) = \smallO{c_I \log(T)}$
...
\begin{leftbar}[theorembar]  % XXX leftbar theorembar, comment if needed
    \emph{No algorithm $\cA$ can achieve} a (mean) regret $R_T^{\cA}(I) = \smallO_I{\log(T)}$ for all Bernoulli problem instances $I \in \cI$.
    \cite[Theorem~2.12]{Slivkins2019}
...
\begin{leftbar}[theorembar]  % XXX leftbar theorembar, comment if needed
    ...
    the regret is asymptotically lower-bounded by $C_I \log(T)$
    \cite[Theorem~2.13]{Slivkins2019},
    or in other words
...
The theoretical results obtained in this thesis are all expressed in terms of the mean regret $R_T^{\cA}$ (in Chapters~\ref{chapter:5} and \ref{chapter:6}), but we quickly mention other measures of performances used in our work.

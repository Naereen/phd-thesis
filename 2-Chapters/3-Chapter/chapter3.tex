...
then we discuss in Section~\ref{sec:3:timeAndMemoryCosts} about the time efficient and memory costs of the some MAB algorithms.
...
A complete Sphinx documentation, for each algorithm and all parts of the codebase, even including the constants in the different configuration files:
...
\begin{figure}[h!]  % [htbp]
	...
	\caption[Example of a single-player simulation showing the average regret and histogram of regrets of $4$ algorithms]{
		Example of a single-player simulation showing the average regret and histogram of regrets of four algorithms. They all perform very well: each algorithm is known to be order-optimal (\ie, its regret is proved to match the lower-bound up-to a constant), and each but UCB is known to be optimal (\ie, with the constant matching the lower-bound).
	}
...
\begin{figure}[h!]  % [htbp]
	...
	\caption{Histogram of regret for the same experiment.
        For instance, Thomson sampling is very efficient in average (in yellow), and UCB shows a larger variance (in red).
    }
...
For each of them, we give a bibliographic reference, that corresponds to the most recent article studying it and not the first one which introduced it.
...
We focussed on the most well known algorithms, and two others that are efficient but less known (AdBandits and BESA),
...
\begin{itemize}
    ...
    \item Total memory cost $\cM_T$ should scale as $\cO(K \log(T))$.
\end{itemize}

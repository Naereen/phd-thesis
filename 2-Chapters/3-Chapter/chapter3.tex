...
It uses \texttt{for} loops and environment variables from Bash, and not Python, but we could also have done the same by writing a Python script that calls the \texttt{main.py} script for the different values of $T$ and $K$.
...
$ for T in $(seq 5000 5000 50000); do \
      ...
      DEBUG=False NOPLOTS=False SAVEALL=True N_JOBS=-1 \
          N=1000  BAYES=True    T=$T         K=$K \
          python3 main.py configuration.py \
      && cp logs/main_py3_log.txt \
              logs/main_py3_log__N1000_BAYES_T{T}_K{K}.txt \
  done; done
    \end{minted}

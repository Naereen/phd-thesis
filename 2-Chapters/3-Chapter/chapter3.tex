...
It clearly illustrates that using more jobs than the number of CPU is suboptimal, and that as soon as the number of repetitions is large enough, using one job by available CPU core (\ie, here $4$ jobs) gives a significant speed-up time.
...
% \begin{footnotesize}
    ...
    \textbf{Repetitions} $\;$ \textbackslash $\;$ \texttt{N\_JOBS} & $1$ & $4$ & $20$ \\
        ...
        $500$ repetitions & $2944 \;\text{s}$ & $1530 \;\text{s}$ & $1346 \;\text{s}$ \\
        \hline
    \end{tabular}

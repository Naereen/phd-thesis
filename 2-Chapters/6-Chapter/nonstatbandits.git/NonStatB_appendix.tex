...
We state the Pinsker's inequality precisely in Lemma~\ref{lem:6:Pinsker} below, and prove it with simple real analysis arguments\footnote{~The proof is inspired by Lemma~10.2 from \cite{LattimoreBanditAlgorithmsBook} (version $1699$ of January $2019$).}.

\begin{lemma}[Pinkser'es inequality]\label{lem:6:Pinsker}
...
\begin{proof}
    ...
    Let's compute its derivative, $g'(x) = (\frac{\partial \kl}{\partial 2})(p, p + x) - 4x$.
    And
    ...
    We can verify that $g'(0) = -1 + 1 - 4*0 = 0$, and it has the sign of $-x$ on its domain, as $p + x - 1 < 0$ because $x < 1 - p$, and $p + x > 0$.

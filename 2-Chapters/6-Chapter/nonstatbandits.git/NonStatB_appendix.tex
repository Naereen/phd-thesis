...
\TODOL{I need to give a proof of the result that links the GLR with a ratio of two sup and the GLR with a sup of parametric kl}
...
\begin{lemma}\label{lem:6:Chernoff2arms}
    ...
    \begin{equation}
        \bP\left(\frac{sr}{s+r}\Big(\widehat{\mu}_{a,s} - \widehat{\mu}_{b,r} - \Delta\Big)^2 \geq u \right) &\leq& 2\exp\left(- \frac{u}{2\sigma^2}\right).
    \end{equation}
\end{lemma}
...
\begin{align}\label{eq:6:Preparation}
	&\bP\left(\frac{sr}{s+r}\Big(\widehat{\mu}_{a,s} - \widehat{\mu}_{b,r} - \Delta\Big)^2 \geq u \right) \nonumber\\
...
Our analysis for the other variant of \GLRklUCB{} also relies on the general regret decomposition~\eqref{eq:6:GeneRegretBound} and follows the same skeleton, with the following appropriate good event.
%
...
%-----------------------------------------------------------------------------
\subsection{Two ideas of numerical optimization}\label{sub:6:IdeasOptimizations}

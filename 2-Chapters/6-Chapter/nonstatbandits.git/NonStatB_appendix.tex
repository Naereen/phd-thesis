...
\TODOL{Maybe I should keep this, and do the same for the confidence level $\delta$}
...
% \TODOL{In only one or two short numerical experiment, for example Pb 1 and 2, I can present in a table the regret obtained by \GLRklUCB, both local and global or only local, for different values of the threshold functions.
...
We compare the following threshold functions that can be used in the B-GLRT test used for the \GLRklUCB{} algorithm (see the detail in equation \eqref{eq:whenDoesGLRklUCBrestart} and in Algorithm~\ref{algo:6:GLRklUCB}).

\begin{enumerate}
    \item
    The first variant is the default one, and it is the one we advised to use in practice for \GLRklUCB{}.
    It is very simple to compute numerically:
    \[ \beta_1(t, \delta) := \log\left(\frac{3 t^{3/2}}{\delta}\right) = \log(\frac{1}{\delta}) + \log(3) + 3/2 \log(t). \]

    \item
    The second variant is smaller and does not use this power $3/2$:
    \[ \beta_2(t, \delta) := \log(\frac{1}{\delta}) + \log(1 + \log(t)). \]

    \item
    The third variant is using the function $\cT$, as introduced by \eqref{def:6:function_T}.
    The function $\cT$ is computed with a numerical approximation\footnote{~The Lambert $\cW$ function is provided in Python with \texttt{scipy.special.lambertw} from the \texttt{scipy} module \cite{scipy}.} of the Lambert function $\cW$, as explained in Section~\ref{par:6:falseAlarm},
    \[ \beta_3(t, \delta) := 2 \mathcal{T}\left(\frac{\log(2 t^{3/2}) / \delta}{2}\right) + 6 \log(1 + \log(t)). \]

    \item
    The forth variant is using the function $\tilde{\mathcal{T}}(x) = x + 4 \log(1 + x + \sqrt{2x})$, as a simple approximation of $\cT(x)$, which is valid and quite accurate as soon as $x \geq 5$,
    \[ \beta_4(t, \delta) := 2 \tilde{\mathcal{T}}(x)\left(\frac{\log(2 t^{3/2}) / \delta}{2}\right) + 6 \log(1 + \log(t)). \]
\end{enumerate}

As before, we consider the three problems $1$, $2$ and $4$, with time horizon $T=5000$,
and we present in Table~\ref{table:6:exploringDifferentThresholdFunctions} the mean results of $1000$ independent runs.
...
\begin{table}[ht]
    \centering
    \begin{tabular}{c|cccccc}
        \textbf{Choice of threshold function} \textbackslash \textbf{Problems} & Problem $1$ & Problem $2$ & Problem $4$ \\
        \hline
...
Therefore, our choice of threshold function $\beta_1$ for our proposal \GLRklUCB{} presented in Algorithm~\ref{algo:6:GLRklUCB} is validated by these experiments, and this choice gives good performance for our proposal.
...
% \TODOL{In only one or two short numerical experiment, for example Pb 1 and 2, I can present in a table the regret obtained by \GLRklUCB, both local and global or only local, for different options of the forced exploration mechanism (uniform random, tracking, deterministic).
%     TODO run and finish the experiments!
% }
...
As before, we consider the three problems $1$, $2$ and $4$, with time horizon $T=5000$,
and we present in Table~\ref{table:6:exploringDifferentForcedExplorationMechanisms} the mean results of $1000$ independent runs.
...
\begin{table}[ht]
    \centering
    \begin{tabular}{c|cccccc}
        \textbf{Choice of exploration mechanism} \textbackslash \textbf{Problems} & Problem $1$ & Problem $2$ & Problem $4$ \\
        \hline
        Deterministic exploration (\textbf{Local})   & $\mathbf{68 \pm 33}$ & $116 \pm 36$ & $99 \pm 33$ \\
        Deterministic exploration (\textbf{Global})  & $97 \pm 28$ & $134 \pm 36$ & $131 \pm 32$ \\
        Uniform random exploration (\textbf{Local})  & $74 \pm 30$ & $\mathbf{108 \pm 33}$ & $106 \pm 31$ \\
        Uniform random exploration (\textbf{Global}) & $91 \pm 30$ & $134 \pm 33$ & $129 \pm 33$ \\
        Tracking-based exploration (\textbf{Local})  & $73 \pm 32$ & $104 \pm 33$ & $\mathbf{89 \pm 29}$ \\
        Tracking-based exploration (\textbf{Global}) & $96 \pm 26$ & $133 \pm 32$ & $120 \pm 30$
    \end{tabular}
    \caption{Mean regret $\pm$ $1$ standard-deviation, for different choices of exploration mechanisms, on three problems of horizon $T=5000$, for \GLRklUCB.}
    \label{table:6:exploringDifferentForcedExplorationMechanisms}
\end{table}

As expected, all options give similar results, we each of the three options was found to outperform the two other in one of the three problems considered for this experiments (problems $1$, $2$ and $4$).
...
Therefore, our choice of the deterministic exploration scheme for our proposal \GLRklUCB{} presented in Algorithm~\ref{algo:6:GLRklUCB} is validated by these experiments, and this choice gives good performance for our proposal.

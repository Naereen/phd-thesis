...
%
We refer to \cite{Basseville93} for the generic proof, but we prove here the specific case of Bernoulli distributions.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= s \left( \widehat{\mu}_{1:s} \log(\mu_0) + (1 - \widehat{\mu}_{1:s}) \log(1-\mu_0) \right) \\
        & \;\;\;\; + (n-s) \left( \widehat{\mu}_{s+1:n} \log(\mu_1) + (1 - \widehat{\mu}_{s+1:n}) \log(1-\mu_1) \right).
    \end{align*}
    ...
    \begin{align*}
        ...
        &= \exp \Bigl[ \sup\limits_{\mu_0,\mu_1,\tau < n} \log\Bigl\{ \frac{\ell(X_1, \ldots, X_n ; \mu_0,\mu_1,\tau)}{ \exp\left[ n \left( \widehat{\mu}_{1:n} \log(\widehat{\mu}_{1:n}) + (1 - \widehat{\mu}_{1:n}) \log(1-\widehat{\mu}_{1:n}) \right) \right] } \Bigr\} \Bigr],  \\
        &= \exp \Bigl[ \sup\limits_{\mu_0,\mu_1,\tau < n}
            s \left( \widehat{\mu}_{1:s} \log(\mu_0) + (1 - \widehat{\mu}_{1:s}) \log(1-\mu_0) \right) \\
            & \hspace{20pt} + (n-s) \left( \widehat{\mu}_{s+1:n} \log(\mu_1) + (1 - \widehat{\mu}_{s+1:n}) \log(1-\mu_1) \right) \\
            & \hspace{20pt} - n \left( \widehat{\mu}_{1:n} \log(\widehat{\mu}_{1:n}) + (1 - \widehat{\mu}_{1:n}) \log(1-\widehat{\mu}_{1:n}) \right)
        \Bigr].
    \end{align*}
    By linearity and independence, we can separate the optimization problem in two optimizations problems for $\mu_0,\tau$ and $\mu_1,\tau$, and then we can first solve for $\mu_0$ (resp. $\mu_1$).
    As $n = s + (n-s)$, the right hand side (negative) part involving $\widehat{\mu}_{1:n}$ is distributed in the two left hand side (positive) terms,
    ...
    \begin{align*}
        ...
        &= \exp \Bigl[ \sup\limits_{1 < s < n} \Bigl[
            s \bigl(
                    ...
                    - \widehat{\mu}_{1:n} \log(\widehat{\mu}_{1:n}) + (1 - \widehat{\mu}_{1:n} \log(1-\widehat{\mu}_{1:n})
            \bigr) \\
            & \;\;\;\; + (n-s) \bigl(
        ...
        \Bigr] \Bigr].
    \end{align*}
    We recognize the expressions of $s \times \kl(\widehat{\mu}_{1:s}, \widehat{\mu}_{1:n})$

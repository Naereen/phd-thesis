...
the two processes $\bm{X}(s)$ and $\bm{Y}(s)$ are called $g$-DCC for Doob-Cram\'er-Chernoff,
if $\bm{X}(s) \eqdef s \, \kl(\widehat{\mu}_s,\mu) - 3\ln(1+\ln(s))$ and $\bm{Y}(s) \eqdef r \, \kl(\widehat{\mu}_r,\mu) - 3\ln(1+\ln(r))$,
as Doob's inequality can be applied in combination with the Cram\'er-Chernoff method to obtain deviation inequalities that are uniform in time.
...
\begin{leftbar}[lemmabar]  % XXX leftbar lemmabar, comment if needed
    Let $\widehat{\mu}_{i,s}$ be the empirical mean of $s$ \iid{} observations with mean $\mu_i$, for $i \in \{a,b\}$, that are $\sigma^2$-sub-Gaussian. Define $\Delta \eqdef \mu_a - \mu_b$. Then for any $s,r > 0$, we have
    \begin{equation}

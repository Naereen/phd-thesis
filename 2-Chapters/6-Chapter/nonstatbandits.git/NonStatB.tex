...
% \emph{Problem-independent} upper bounds of the form $R_T=\Omega(\sqrt{KT})$ (with no hidden constant depending on the arms distributions) have also been established for stochastic algorithms, like MOSS, or \klUCB-Switch by \cite{GarivierHadiji2018}, while \klUCB{} is known to enjoy a sub-optimal $\cO(\sqrt{KT\ln(T)})$ problem-independent regret.
...
% \TODOL{I need to write a paragraph about using the ``adversarial setting'' for piece-wise stationary. It is very important to state that \ExpThreeS{} from \cite{Auer02NonStochastic} (Section~8) achieves $\bigO{\sqrt{K \Upsilon_T T}}$ regret with NO prior knowledge of $T$ and $\Upsilon_T$.}
...
It is characterized by a set of $K$ arms, and an horizon $T$.
...
Note than when a break-point occurs, we do not assume that all the arms means  change, but that \emph{there exists} an arm whose mean has changed.
Depending on the application, many scenario can be meaningful: changes can occur on all arms simultaneously (due to some exogenous event), or only one or a few arms change at each break-point.
...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=1.09\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_1.pdf}
...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=1.09\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_2.pdf}
...
To the best of our knowledge, all the previous works are based on the idea of combining a classical bandit policy, such as Thompson sampling, \UCB{} or \ExpThree, with a strategy to account for changes in arms' distributions.
...
% However, no theoretical guarantees are given for this strategy, and our experiments did not really confirm the robustness to $\gamma$.
%
...
%
The \ExpThreeR{} algorithm from \cite{Allesiardo15,Allesiardo17} combines a CD algorithm with \ExpThree, and the history of all arms are reset as soon as a suboptimal arm is detecting to become optimal.
...
and we found in our experiments that \PHT-\UCB{} performs very similarly to \CUSUMUCB.
...
%
Both approaches also introduced a mechanism to force a uniform exploration of each arm, parametrized by $\omega\in(0,1)$ (note that it is called $\alpha$ in both papers, but we rename it $\omega$ in this chapter to avoid clutter with $\alpha$ of $\UCB_1$ in \eqref{eq:2:UCB_index}).
...
When all their parameters are tuned correctly, both approaches are proven to achieve good regret upper-bounds, of order $\cO(K \sqrt{\Upsilon_T T \log(K T)})$.
...
% Our algorithmAdSwitch(shown as Algorithm 1) proceeds in episodes, where a new episode starts when the algorithm detects a change in one of the arms. Each episode starts with an estimation phase (Step 2), in which both arms are chosen alternatingly until their means can be distinguished (Step 3). After the estimation phase, the algorithm exploits the empirical best arm (Step 5). However, in order to detect changes, at each step with some probability (Step 4) an exploration phase is started that checks whether a change has occurred (comparing the resulting empirical means to that obtained from the estimation phase). During such exploration phases the algorithm checks for changes of certain magnitudes (denoted by di). Note that our algorithm does not require knowledge of the number of changesL. The following theorem provides an upper bound on the expected regret of our algorithm
...
% Note that this algorithm is introduced for a different setting, where the quantity of interest is not $\Upsilon_T$
...
The slowly-varying model is quite different from the setting we are considering in this chapter, and even if the most recent works on this model are very interesting \cite{WeiSrivastava18Abruptly}, we preferred to focus on the piece-wise stationary (also called abruptly changing) model,
...
it only needs to know before hand that $T=5000$ and $\Upsilon_T=4$ for these examples, but it does not need to know the amplitude of changes $\Delta^{\text{change}} = \max_{k,t} |\mu_k(t) - \mu_k(t+1)|$ nor the optimality gap or a bound on the optimality gap at any time step $\Delta^{\text{opt}} = \max_{k,k'} |\mu_k(t) - \mu_{k'}(t)|$.
...
As explained in Section~\ref{par:2:normalizationTrick}, our work can be applied to any bounded distribution without loss of generality, as a reward in $r\in[a,b]$ can clearly be mapped to $[0,1]$ simply by using $r' = (r-a)/(b-a)$. We assume that both the decision maker and the algorithm know beforehand the values of $a$ and $b$.
...
% , based on the quadratic divergence $d(x,y) = 2(x-y)^2$.
%
...
% %
% The B-GLRT test is defined in Definition~\ref{def:6:GLRDef}.
...
\begin{leftbar}[lemmabar]  % XXX leftbar lemmabar, comment if needed
    ...
    % and has an average of $\mu_0$ for all $t$.
    ...
    % given in Definition~\ref{def:6:GLRDef}
...
% (which is the Kullback-Leibler divergence between two distributions of means $x$ and $y$).
%
...
\begin{leftbar}[lemmabar]  % XXX leftbar lemmabar, comment if needed
    ...
    % and $X_t$ has an average of $\mu_0$ for all $t \leq \tau$, and $\mu_1$ for all $t > \tau$, with $\mu_0 \neq \mu_1$,
...
For a threshold $\beta$ chosen as in the Lemma~\ref{lem:6:FalseAlarm}, a consequence of the Lemma~\ref{lem:6:Delay} is that if the break-point $\tau$ happens after about $\cO(\Delta^{-2} \ln(1/\delta))$ samples, the detection time will be of the same magnitude (with high probability).
%
...
We now present our \GLRklUCB{} algorithm, which combines a bandit algorithm with a change-point detector running on each arm.
...
\begin{figure}[h!]
    ...
    \begin{algorithm}[H]
        ...
        \caption{The \GLRklUCB{} algorithm, with \textbf{Local} or \textbf{Global} restarts.}
...
%
Our proposal \GLRklUCB{} (lines $3$-$5$) generalizes the deterministic exploration proposed for \MUCB{} by \cite{CaoZhenKvetonXie18}, whereas \CUSUMUCB{} performs a uniform random exploration.
...
Assumption~\ref{ass:6:LongPeriods} is easy to interpret, as Assumption~\ref{ass:6:LongPeriodsGlobal}, but it is non standard the literature, and to the best of or knowledge, our analysis is the first one to consider such assumption on the problem difficulty.
%
...
The tricky part of both proofs, which crucially exploits Assumption~\ref{ass:6:LongPeriodsGlobal} or \ref{ass:6:LongPeriods}, is actually to obtain an upper bound on $\bP(\cE_T^c)$. For example for \emph{local restarts} (Theorem~\ref{thm:6:mainRegretBound}), the good event is defined as
% \begin{equation}
...
Finally by controlling terms $(a)$ and $(b)$, the ``bad event'' is unlikely: $\bP(\cE_T^c) \leq 2\EffChange \delta$.
By putting together the three pieces, that are a bound on $(A)$, $(B)$ and $\bP(\cE_T^c)$, we obtain the desired finite-time upper-bound on the regret of our proposal \GLRklUCB.
...
The last inequality comes from a sum-integral comparison, as $\int\limits_{1}^T \frac{1}{t \ln(t)} \mathrm{d}t = \ln(\ln(T))$.
...
Recall that $\cC^{(i)}$ defined in \eqref{def:6:EventCiGlobal} is the event that all the break-points up to the $k$-th have been correctly detected.
...
$\widetile{\mu}_{1:x} = \widetile{\mu}_{1:y+z} = \frac{1}{y+z} \sum_{i=1}^{y+z} Z_i = \frac{1}{y+z} \left( y \widetile{\mu}_{1:y} + z \widetile{\mu}_{y+1:z} \right)$,
...
Using Pinsker's inequality, and introducing the gap $\Delta_k^{(i)} \eqdef \mu_k^{(i-1)} - {\mu}_k^{(i)}$ (which is such that $\Delta^{(i)} = |\Delta_k^{(i)}|$), one can write
%
\begin{align}
...
We include in this study two algorithms designed for the classical stationary MAB, \klUCB{} \cite{Garivier11KL}, and Thompson sampling \cite{AgrawalGoyal11,Kaufmann12Thompson},
...
% We show in Appendix~\ref{sec:6:choosingAlpha0}
...
% show in Appendix~\ref{sec:6:choosingDelta}
...
As shown below, in the experiments summary, the three algorithms based on change-point detection obtain sublinear regret, but they have different behaviors.
...
On this problem, while \MklUCB{} detects the same changes as \GLRklUCB, with larger but comparable delays, it is seen to obtain a larger regret than the two \GLR{} variants, simply because the tuning of its forced exploration probability (\ie, $\omega$) makes it explore suboptimal arms more often.
...
The values change, but the ranking stays the same: our proposal outperforms both \CUSUMklUCB{} and \MklUCB, and the test based on \CUSUM{} seems more efficient than the test based of \MklUCB{} in the problems at hand.
...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=1.05\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_5.pdf}
...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=1.05\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_4.pdf}
...
% In practice, this problem is very hard and and

\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=1.05\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_6.pdf}
...
% , except Thompson Sampling which has a large tail due to its large mean regret on this (easy) non-stationary problems.
...
As mentioned at the end of Chapter~\ref{chapter:5}, an interesting direction of future work is to study non-stationary distributed multi-players bandits.

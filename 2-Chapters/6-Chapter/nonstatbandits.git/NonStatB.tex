...
\TODOL{Include in Appendix a formal statement and a proof of Pinsker's inequality?}
...
%
For the B-GLRT, numerical simulations suggest that the threshold \eqref{def:6:beta} is a bit conservative (see Appendix~\ref{sec:6:exploringDifferentThresholdFunctions}), and in practice we recommend to keep only the leading term and use $\beta(n,\delta) = \ln(3n\sqrt{n}/\delta)$.
...
with the simpler value $\beta(n,\delta) = \ln(3n\sqrt{n}/\delta)$,
as well as a few other choices.
The sum-up of these simulations is that it is enough to use a simpler and more explicit threshold to have an efficient change-point detection algorithm, and an efficient \GLRklUCB{} policy.
...
Some new notations are needed to state a regret bound for \GLRklUCB{} using local changes. We let $\tau_i^{(\ell)}$ denote the position of the $\ell$-th change point \emph{for arm $i$}: $\tau_i^{(\ell)} = \inf \{ t > \tau_i^{(\ell - 1)} : \mu_i(t) \neq \mu_i(t+1)\}$,

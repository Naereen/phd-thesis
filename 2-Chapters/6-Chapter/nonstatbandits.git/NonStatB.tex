...
\TODOL{I need to write a paragraph about using the ``adversarial setting'' for piece-wise stationary. It is very important to state that EXP3.S from \cite{Auer02NonStochastic} (Section~8) achieves $\bigO{\sqrt{K \Upsilon_T T}}$ regret with prior knowledge of $T$ and $\Upsilon_T$.}
...
\TODOL{Talk about NEWMA and its possible application to generalize/improve D-UCB, \cite{Keriven2018}.}
...
\begin{proof}
    ...
    \TODOL{We have to give the proof? I was fine with this sentence...}
\end{proof}
...
by giving the times at which they detected a change and reinitialized their memory of observations of one or all the arms.
We also give the number of observations (since the last restart) of the arm on which the change was detected.

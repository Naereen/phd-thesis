...
We discuss numerical experiments in Section~\ref{sec:6:NumericalExperiments}, with more details in the Appendix.
...
We denote by $\mu_k(t) \eqdef  \bE[Y_{k,t}]$ the mean reward of arm $k$ at round $t$. At each round $t$, a decision maker has to select an arm $A(t)\in[K]$, based on past observation and receives the corresponding reward $r(t) = Y_{A(t),t}$.
...
For instance, for a cognitive radio application in a IoT-like network, we can imagine that all the objects of one company use a fixed (or a subset of) channel(s), and on a particular day in a city, if the company deploys a lot of new objects, then one (or some) channel(s) see their mean occupancy rates abruptly change.
...
Active strategies monitor the arms' rewards, by using change detection algorithms \cite{Basseville93}, and reset the history of pulls and rewards of one or all the arms as soon as a change is detected.
...
\end{equation}
%
where for $k \leq k'$, $\widehat{\mu}_{k:k'} \eqdef \frac{1}{k - k' + 1} \sum\limits_{l=k}^{k'} X_l$ denotes the average of the observations $X_l$ collected between the instants $k$ and $k'$.
...
We also introduce $i^* = \arg\max_k \mu_k^{(i)} $ and the largest gap at break-point $i$ as $\Delta^{(i)} \eqdef \max_{k\in[K]} |\mu_k^{(i)} - \mu_k^{(i-1)}| >0$.
...
\begin{leftbar}[corollarybar]  % XXX leftbar corollarybar, comment if needed
    ...
    \begin{enumerate}
        \item Choosing $\alpha = \sqrt{\ln(T) / T}$, $\delta = 1 / \sqrt{T}$ gives
        \begin{equation}
            R_T = \bigO{\frac{K}{\left(\Delta^{\text{change}}\right)^2} \EffChange\sqrt{T\ln(T)} + \frac{1}{\left(\Delta^{\text{opt}}\right)^2} \EffChange\ln(T)},
        \end{equation}
        \item Choosing $\alpha = \sqrt{\Upsilon_T\ln(T) / T}$, $\delta = 1 / \sqrt{\Upsilon_T T}$ gives
        \begin{equation}
...
%
\begin{eqnarray*}
    n_k(\tau^{(i)})& \in& \left\{ \left\lfloor \frac{\alpha}{2K} (\tau^{(i)}-\tau^{(i-1)} )\right\rfloor, \dots, \tau^{(i)}-\tau^{(i-1)} \right\} \eqdef \cI_k.
\end{eqnarray*}

...
\begin{small}
    \begin{equation}\label{eq:whenDoesGLRklUCBrestart}
        \sup_{s \in [n-1]} \left[s \times \kl \left(\frac{1}{s}\sum_{i=1}^s Z_k , \frac{1}{n}\sum_{i=1}^n Z_k\right) + (n-s) \times \kl \left(\frac{1}{n-s}\sum_{i=s+1}^t Z_k , \frac{1}{n}\sum_{i=1}^n Z_k\right)\right] \geq \beta(n,\delta),
    \end{equation}
\end{small}
%
...
$\widehat{\mu}_k(t) \eqdef \frac{1}{n_k(t)} \sum_{s=\tau_k(t)+1}^{t}Y_{k,s} \indic(A(s)=i)$
...
%
It requires that any two consecutive change-points \emph{on a given arm} are sufficiently spaced (relatively to the magnitude of those two change-points). Under that assumption, Theorem~\ref{thm:6:mainRegretBound} provides a regret upper bound that scales with similar quantities as that of Theorem~\ref{thm:6:mainRegretBoundGlobal}, except that the number of break-points $\Upsilon_T$ is replaced with the \emph{total} number of change-points $\EffChange \eqdef \sum_{k=1}^K \NCk \leq K \Upsilon_T$.
...
The last inequality comes from a sum-integral comparison, as $\int_{1}^T \frac{1}{t \ln(t)} = \ln(\ln(T))$.
...
\begin{align*}
    ...
    & \leq \sum_{i=1}^{\Upsilon_T} d_k^{(i)}+ \sum_{k=1}^K \bE\Big[\indic(\cE_T) \sum_{t=\widehat{\tau}^{(i)}+1}^{\tau^{(i+1)}}\left(\mu_{k^*}^{(i)} - \mu_k^{(i)}\right)\indic\left(A(t)=i,\UCB_k(t) \geq \mu_{k^*}^{(i)}\right)\Big] \\
...
\end{align*}
%
The final result $\bP(\cE_T^c) \leq (K+1)\delta$ follows by proving that $(a) \leq K \delta$ and $(b)\leq \delta$, as detailed below.
...
%
\begin{align*}
    (a) & \leq \bP\left(\exists k\in[K], \exists s < t \leq n_k(\tau_k^{(i)}) : s \, \kl\left(\widetilde{\mu}_{k,1:s}^{(i-1)},\widetilde{\mu}_{k,1:t}^{(i-1)}\right) + (t - s) \, \kl\left(\widetilde{\mu}_{k,s+1:t}^{(i-1)},\widetilde{\mu}_{k,1:t}^{(i-1)}\right) > \beta(t, \delta) \;|\; \cC^{(i-1)}\right) \\
    & \leq \sum_{k=1}^K\bP\left(\exists s < t : s \, \kl(\widehat{\mu}_{1:s},\mu_k^{(i-1)}) + (t - s) \, \kl(\widehat{\mu}_{s+1:t}, \mu_k^{(i-1)}) > \beta(t, \delta)\right),
\end{align*}
where teh last inequality comes from a union bound on $k\in[K]$,
...
%
From the definition of $\Delta^{(i)}$, there exists an arm $k$ such that $\Delta^{(i)} \eqdef |\mu_k^{(i)} - \mu_k^{(i-1)}|$. We shall prove that it is unlikely that the change-point detector associated to $i$ does not trigger within the delay $d^{(i)}$.
...
as well as an ``oracle'' version of \klUCB, that we call Oracle-Restart. This algorithm knows the exact locations of the break-points, and restarts \klUCB{} at those locations (without any delay).
...
%
Additionally, we include the \ExpThreeS{} algorithm from \cite{Auer02NonStochastic}, setting its parameters as in Corollary 8.3 of the paper, based on a prior knowledge of $T$ and $\Upsilon_T$, using $\omega=1/T$ and $\gamma = \min(1, \sqrt{K (\Upsilon_T \log(KT) + \e) / ((\e-1)T)})$.
...
%
First, we show the mean regret as a function of time (\ie, $R_t$ for $1 \leq t \leq T$), for $9$ of the considered algorithms (as they are outperformed by the others, \ExpThreeS{} and Discounted-\klUCB{} are not included to avoid clutter).
%

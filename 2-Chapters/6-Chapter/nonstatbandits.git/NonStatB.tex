...
%
With the exploration function $f(t)$ defined as $\ln(t) + 3 \ln(\ln(t))$ if $t>1$ and $f(t)=0$ otherwise,
...
% We highlight that our results are all non-asymptotic, contrarily to what is given for \MUCB{} \cite{CaoZhenKvetonXie18} and \CUSUMUCB{} \cite{LiuLeeShroff17}.
...
Introducing $\cD(T,\alpha)$ the (deterministic) set of time steps at which the forced exploration is performed before time $T$ (see lines $3$-$4$ in Algorithm~\ref{algo:6:GLRklUCB}), one can write, using notably that $\mu_{i^*_t} - \mu_{I_t} \leq 1$, due to the bounded rewards,
\begin{align*}
...
that all the changes up to the $\ell$-th have been detected, a union bound yields the following decomposition
%
\begin{align}
...
The proof of \GLRklUCB{} with Local Changes is given in Appendix~\ref{proof:6:mainRegretBound}, as it is quite similar to the proof for Global changes given above.

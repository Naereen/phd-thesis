...
%
In this model, described in full details in Section~\ref{sec:6:BanditSetting}, the (random) reward of arm $k$ at round $t$ has some mean $\mu_k(t)$, that is constant on intervals between two \emph{breakpoints}, and the regret is measured with respect to the \emph{current} best arm $k_t^* = \arg\max_i \mu_k(t)$.
...
We denote by $\mu_k(t) :=  \bE[Y_{k,t}]$ the mean reward of arm $k$ at round $t$. At each round $t$, a decision maker has to select an arm $A(t)\in[K]$, based on past observation and receives the corresponding reward $r(t) = Y_{A(t),t}$.
At time $t$, we denote by $k_t^*$ an arm with maximal expected reward, \ie, $\mu_{k_t^*}(t) = \max_k \mu_k(t)$, called an optimal arm (possibly not unique).

A policy $\cA$ chooses the next arm to play based on the sequence of past plays and obtained rewards.
Like for stationary problems (see Definition~\ref{def:2:regret}), the performance of $\cA$ is measured by its (piece-wise stationary) \emph{regret}, the difference between the expected reward obtained by an oracle policy playing an optimal arm $k_t^*$ at time $t$, and that of the policy $\cA$
...
\begin{equation}\label{def:6:nti}
    R_T^{\cA} = \E\left[\sum_{t=1}^T \left(\mu_{k_t^*}(t) - \mu_{A(t)}(t)\right)\right].
\end{equation}


In the piece-wise \iid{} model, we furthermore assume that there is a (relatively small) number of \emph{breakpoints}, denoted by $\Upsilon_T:=\sum_{t=1}^{T-1} \indic\left(\exists k\in[K]: \mu_k(t) \neq \mu_k(t+1)\right)$.
If we denote $\tau^{(0)} = 0$, we define the $i$-th breakpoint recursively by
...
$\NCk := \sum_{t=1}^{T-1} \indic\left(\mu_k(t) \neq \mu_k(t+1)\right)$,
for which it clearly holds that
$\NCk \leq \Upsilon_T$, but there can be an arbitrary difference between these two quantities for some arms. Letting $C_T := \sum_{k=1}^K \NCk$ be the total number of change-points on the arms, one can have $C_T \in \{ \Upsilon_T, \dots, K\Upsilon_T \}$.
...
% \CUSUMUCB{} uses a randomized exploration, by sampling arm $k$ with probability $\alpha/K$ at each time step,
...
Instead, it uses geometrically decreasing magnitudes $d_k = 2^{-i}$, meaning that even difficult changes should be detected, possibly in a later episode, as each episode only consider a few different values of $d_k$.
...
\begin{equation}\label{eq:6:firstDefGLRT}
    \GLR(n) :=\frac{\sup\limits_{\mu_0,\mu_1,\tau < n}\ell(X_1, \dots, X_n ; \mu_0,\mu_1,\tau)}{\sup\limits_{\mu_0}\ell(X_1, \dots, X_n ; \mu_0)},
\end{equation}
%
...
\begin{smallproof}
    ...
    We have $\ell(X_1, \dots, X_n ; \mu_0) = \prod_{i=1}^n \ell(X_k ; \mu_0)$ by independence of the observations $X_k$,
    and $\ell(X_k ; \mu_0) = \mu_0^{X_k} (1-\mu_0)^{1-X_k}$ for Bernoulli distributions.
    Therefore, taking the logarithm gives
    \begin{align*}
        \log\ell(X_1, \dots, X_n ; \mu_0)
        &= \sum_{i=1}^n X_k \log(\mu_0) + (1-X_k) \log(1-\mu_0) \\
        &= \log(\mu_0) \times \left( \sum_{i=1}^n X_k \right) + \log(1-\mu_0) \times \left( n - \sum_{i=1}^n X_k \right) \\
    ...
    \begin{equation}\label{eq:6:solutionPbOpt_for_1n}
        \sup_{\mu_0} \log\ell(X_1, \dots, X_n ; \mu_0) = n \left( \widehat{\mu}_{1:n} \log(\widehat{\mu}_{1:n}) + (1 - \widehat{\mu}_{1:n}) \log(1-\widehat{\mu}_{1:n}) \right).
    \end{equation}
    ...
    %
    \begin{align*}
        & \log\ell(X_1, \dots, X_n ; \mu_0,\mu_1,\tau) \\
        &= \sum_{i=1}^{\tau} X_k \log(\mu_0) + (1-X_k) \log(1-\mu_0) + \sum_{i=\tau+1}^n X_k \log(\mu_1) + (1-X_k) \log(1-\mu_1) \\
    ...
    By definition, $n \; \widehat{\mu}_{1:n} = \sum_{i=1}^n X_k = s \;\widehat{\mu}_{1:s} + (n-s) \;\widehat{\mu}_{s+1:n}$, so the right hand side (negative) part involving $\widehat{\mu}_{1:n}$ can be distributed in the two left hand side (positive) terms,
...
\begin{leftbar}[lemmabar]  % XXX leftbar lemmabar, comment if needed
    ...
    % Assume that there exists $\mu_0 \in [0,1]$ such that $\bE[X_t] = \mu_0$ and that $X_k \in [0,1]$ for all $i$.
...
\begin{leftbar}[lemmabar]  % XXX leftbar lemmabar, comment if needed
    $(X_k)_{i \in \N^*}$ and $(Y_k)_{i \in \N^*}$ two independent \iid{} processes with respective means $\mu$ and $\mu'$ such that
    ...
    Let $\kl(\mu,\mu') = \KL(\nu^\mu,\nu^{\mu'})$ be the divergence function associated to that exponential family. Introducing the notation $\widehat{\mu}_s = \frac{1}{s}\sum\limits_{i=1}^s X_k$ and $\widehat{\mu}'_r = \frac{1}{r}\sum\limits_{i=1}^r Y_k$, it holds that for every $s,r \in \N^*$,
...
\begin{figure}[h!]
    ...
    \begin{algorithm}[H]
        ...
        \For{$t=1,2,\dots, T$}{
            ...
            \Else{
                $A(t) = \arg\max\limits_{k \in [K]} \mbox{UCB}_k(t)$ as defined in \eqref{def:6:UCB}
            }
            \mbox{Play arm } $A(t)$ \mbox{and receive the reward } $Y_{A(t),t}$ : $n_{A(t)} = n_{A(t)} + 1; Z_{A(t), n_{A(t)}} = Y_{A(t),t}$ \\
...
\begin{equation}\label{def:6:UCB}
    \UCB_k(t) := \max \Bigl\{ q\in[0,1] : n_k(t) \times \kl\left(\widehat{\mu}_k(t),q\right) \leq f(t - \tau_k(t)) \Bigr\}.
\end{equation}
...
$f(t-\tau_k(t))$ is replaced by $f(n_t)$ with $n_t = \sum_{k=1}^K n_k(t)$.
...
More precisely, \CUSUMUCB{} samples arm $k$ with probability $\alpha/K$ at each time step,
...
\begin{leftbar}[propositionbar]  % XXX leftbar propositionbar, comment if needed
    ...
    \begin{equation}
        n_k(t) - n_k(s) \geq \left\lfloor \frac{\alpha}{K} (t-s) \right\rfloor.
    \end{equation}
...
\begin{smallproof}
    ...
    Lines~$3$-$4$ state that $A(u) = u \mod \lceil \frac{K}{\alpha} \rceil$
    ...
    \begin{align*}
        ...
        & \geq \sum_{u=s+1}^{t} \indic\left(A(u) = k, A(u) = u \mod \left\lceil \frac{K}{\alpha} \right\rceil\right) \\
        & \geq \sum_{u=s+1}^{t} \indic\left(k = u \mod \left\lceil \frac{K}{\alpha} \right\rceil\right) \\
...
We also introduce $i^* = \arg\max_k \mu_k^{(i)} $ and the largest gap at break-point $i$ as $\Delta^{(i)} := \max_{k\in[K]} |\mu_k^{(i)} - \mu_k^{(i-1)}| >0$.


\begin{assumption}[Enough samples between two global change-points]\label{ass:6:LongPeriodsGlobal}
\begin{leftbar}[assumptionbar]  % XXX leftbar assumptionbar, comment if needed
    Define
    \begin{equation}\label{def:6:DelayGlobal}
        d^{(i)} = d^{(i)}(\alpha,\delta) = \Bigl\lceil \frac{4K}{\alpha\left(\Delta^{(i)}\right)^2}\beta(T,\delta) + \frac{K}{\alpha} \Bigr\rceil.
    \end{equation}
    %
    Then \emph{we assume} that for all change $i \in [\Upsilon_T]$,
...
\begin{leftbar}[theorembar]  % XXX leftbar theorembar, comment if needed
    ...
    \begin{align}
        R_T \;\; \leq \;\; & 2\sum_{k=1}^{\Upsilon_T} \frac{4K}{\alpha \left(\Delta^{(i)}\right)^2}\beta(T,\delta) +\alpha T + \delta (K+1)\Upsilon_T \\
        & + \sum_{k=1}^{\Upsilon_T}\sum_{i : \mu_k^{(i)} \neq \mu_{k^*}^{(i)}} \frac{\left( \mu_{k^*}^{(i)}-\mu_k^{(i)}\right)}{\kl\left(\mu_k^{(i)},\mu_{k^*}^{(i)}\right)}\ln(T) + \bigO{\sqrt{\ln(T)}}. \nonumber
    \end{align}
...
\begin{assumption}[Enough samples between two local change-points]\label{ass:6:LongPeriods}
\begin{leftbar}[assumptionbar]  % XXX leftbar assumptionbar, comment if needed
    Define
    \begin{equation}\label{def:6:DelayLocal}
        d_k^{(\ell)} = d_k^{(\ell)}(\alpha,\delta) = \Bigl\lceil \frac{4K}{\alpha\left(\Delta_k^{(\ell)}\right)^2}\beta(T,\delta) + \frac{K}{\alpha} \Bigr\rceil.
    \end{equation}
    %
    Then \emph{we assume} that for all arm $k$ and all change-point of that arm $\ell \in [\NCk]$, $\tau_k^{(\ell)} - \tau_k^{(\ell-1)} \geq 2\max (d_k^{(\ell)},d_k^{(\ell-1)})$.
\end{leftbar}  % XXX leftbar assumptionbar, comment if needed
\end{assumption}
...
%
It requires that any two consecutive change-points \emph{on a given arm} are sufficiently spaced (relatively to the magnitude of those two change-points). Under that assumption, Theorem~\ref{thm:6:mainRegretBound} provides a regret upper bound that scales with similar quantities as that of Theorem~\ref{thm:6:mainRegretBoundGlobal}, except that the number of breakpoints $\Upsilon_T$ is replaced with the \emph{total} number of change points $\EffChange = \sum_{k=1}^K \NCk \leq K \Upsilon_T$.
...
\begin{leftbar}[theorembar]  % XXX leftbar theorembar, comment if needed
    ...
    %
    \begin{equation}
        R_T \leq 2\sum_{k=1}^K\sum_{\ell=1}^{\NCk} \frac{4K}{\alpha \left(\Delta_k^{(\ell)}\right)^2}\beta(T,\delta) +\alpha T + 2 \delta \EffChange  + \sum_{k=1}^K\sum_{\ell=1}^{\NCk} \frac{\ln(T)}{\kl(\overline{\mu}_k^{(\ell)},{\mu}^*_{i,\ell})} +\bigO{\sqrt{\ln(T)}},
    \end{equation}
    where ${\mu}^*_{i,\ell} = \inf \left\{ \mu_{k_t^*}(t) : \mu_{k_t^*}(t) \neq \overline{\mu}_k^{(\ell)}, t \in [\tau_k^{(\ell)}+1, \tau_k^{(\ell+1)}]\right\}$.
\end{leftbar}  % XXX leftbar theorembar, comment if needed
\end{theorem}
...
%
Recall that in this Chapter, the regret is defined as $R_T = \bE\left[\sum_{t=1}^T(\mu_{k_t^*}(t) - \mu_{A(t)}(t))\right]$.
...
\begin{align*}
 ...
 & \hspace{1cm} \leq  \sum_{t=1}^{T} \indic(t \in \cD(T,\alpha)) + \sum_{t=1}^T (\mu_{k_t^*}(t) - \mu_{A(t)}(t))\indic\left(t \notin\cD(T,\alpha), \UCB_{A(t)}(t) \geq \UCB_{k_t^*}(t)\right) \\
 & \hspace{1cm}\leq  \alpha T + \sum_{t=1}^T \indic\left(\UCB_{k_t^*}(t) \leq \mu_{k_t^*}(t)\right) + \sum_{k=1}^K\sum_{t=1}^T (\mu_{k_t^*}(t) - \mu_k(t))\indic\left(A(t) = k, \UCB_k(t) \geq \mu_{k_t^*}(t)\right).
\end{align*}
%
...
\begin{align}\label{eq:6:GeneRegretBound}
    R_T \leq T \bP\left(\cE_T^c\right) + \alpha T & + \underbrace{\bE\left[\indic(\cE_T) \sum_{t=1}^T \indic\left(\UCB_{k_t^*}(t) \leq \mu_{k_t^*}(t)\right)\right]}_{(A)} \\
...
%
To control $(A)$ and $(B)$, we split the sum over consecutive segments $[\tau^{(i)}+1, \tau^{(i+1)}]$ for global changes and $[\tau^{(i)}_k+1, \tau^{(i+1)}_k]$ for each arm $k$ for local changes, and use elements from the analysis of \klUCB{} of \cite{KLUCBJournal}.
...
\begin{equation}
    \cE_T(\alpha,\delta) = \left(\forall k \in [K], \forall \ell \in [\NCk], \hat{\tau}^{(\ell)}_k \in \left[\tau_k^{(\ell)} + 1, \tau_k^{(\ell)} + d_k^{(\ell)}\right] \right),
\end{equation}
%
where $\widehat{\tau}_k^{(\ell)}$ is defined as the $\ell$-th change detected by the algorithm on arm $k$ and $d_k^{(\ell)}=d_k^{(\ell)}(\alpha,\delta)$ is defined in Assumption~\ref{ass:6:LongPeriods}.
Introducing the event
\[\cC_k^{(\ell)} = \left\{\forall j \leq \ell, \widehat{\tau}_k^{(\ell)} \in \left[\tau_k^{(j)} + 1, \tau_k^{(j)} + d_k^{(j)} \right] \right\},\]
that all the changes up to the $\ell$-th have been detected, a union bound yields this decomposition
%
\begin{align}
    \bP(\cE_T(\alpha,\delta)^c) & \leq \sum\limits_{k=1}^K\sum\limits_{\ell=1}^{\NCk} \underbrace{\bP\left(\widehat{\tau}_k^{(\ell)} \leq \tau_k^{(\ell)} \;|\; \cC_k^{(\ell-1)}\right)}_{(a)} + \sum\limits_{k=1}^K\sum\limits_{\ell=1}^{\NCk} \underbrace{\bP\left(\widehat{\tau}_k^{(\ell)} \geq \tau_k^{(\ell)} + d_k^{(\ell)} \;|\; \cC_k^{(\ell-1)}\right)}_{(b)}.
\end{align}
...
We denote $n_k(t) = \sum_{s=\tau(t)+1}^{t}\indic(A(s) = i)$ the number of selections of arm $k$ since the last (global) restart, and $\widehat{\mu}_k(t) = \frac{1}{n_k(t)}\sum_{s=\tau(t)+1}^{t}Y_{k,s}\indic(A(s) = i)$ their empirical average (if $n_k(t) \neq 0$).
...
\begin{equation}\cE_T(\delta) = \left( \forall k \in [\Upsilon_T], \hat{\tau}^{(i)} \in \left[\tau^{(i)} + 1, \tau^{(i)} + d^{(i)}\right] \right).\label{def:6:GoodEvenGlobal}\end{equation}
Under the good event, all the change points are detected within a delay at most $d^(k)$.
%
From Assumption~\ref{ass:6:LongPeriodsGlobal}, as the period between two changes are long enough, if $\cE_T(\delta)$ holds, then for all change $k$, we have $\tau^{(i)} \leq \hat{\tau}^{(i)} \leq \tau^{(i+1)}$.
...
\begin{align*}
    ...
    & \leq \sum_{k=1}^{\Upsilon_T}\bE\left[ d^{(i)} + \indic(\cE_T) \sum_{t=\hat{\tau}^{(i)}+1}^{\tau^{(i+1)}} \indic\left(n_{k^*}(t) \, \kl\left(\widehat{\mu}_{k^*}(t), \mu_{k^*}^{(i)}\right) \geq f(t - \widehat{\tau}^{(i)})\right)\right] \\
    & \leq \sum_{k=1}^{\Upsilon_T}d^{(i)} + \sum_{k=1}^{\Upsilon_T}\bE\left[\indic(\cC^{(i)}) \sum_{t=\hat{\tau}^{(i)}}^{\tau^{(i+1)}}\indic\left(n_{k^*}(t) \, \kl\left(\widehat{\mu}_{k^*}(t), \mu_{k^*}\right) \geq f(t - \widehat{\tau}^{(i)})\right)\right],
\end{align*}
%
...
\begin{align*}
    &\bE\left[\left.\indic(\cC^{(i)}) \sum_{t=\hat{\tau}^{(i)}}^{\tau^{(i+1)}} \indic\left(n_{k^*}(t) \, \kl\left(\widehat{\mu}_{k^*}(t), \mu_{k^*}^{(i)}\right) \geq f(t - \widehat{\tau}^{(i)})\right) \right| \cF_{\widehat{\tau}^{(i)}}\right]\\
    & = \indic(\cC^{(i)}) \sum_{t=\hat{\tau}^{(i)}}^{\tau^{(i+1)}} \bE\left[\indic\left(n_{k^*}(t) \, \kl\left(\widehat{\mu}_{k^*}(t), \mu_{k^*}^{(i)}\right) \geq f(t - \widehat{\tau}^{(i)})\right) \;|\; \cF_{\widehat{\tau}^{(i)}}\right] \\
    & \leq \indic(\cC^{(i)}) \sum_{t'=1}^{\tau^{(i+1)} - \hat{\tau}^{(i)}} \bP\left(\exists s \leq t' : s \, \kl(\widehat{\mu}_{s},\mu_{k^*}^{(i)}) \geq f(t')\right) \\
...
%
We let $\tilde{\mu}_{k,s}^{(i)}$ denote the empirical mean of the first $s$ observations of arm $k$ made after time $t=\hat{\tau}^{(i)}+1$. Rewriting the sum in $t$ as the sum of consecutive intervals $[\tau^{(i)}+1, \tau^{(i+1)}]$,
%
\begin{align*}
    (B) & \leq \bE\Big[\indic(\cE_T)\sum_{k=1}^{\Upsilon_T} \sum_{t=\tau^{(i)}}^{\tau^{(i+1)}}\left(\mu_{k^*}^{(i)} - \mu_{A(t)}^{(i)}\right)\indic\left(\UCB_{A(t)}(t) \geq \mu_{k^*}^{(i)}\right)\Big] \\
     & \leq \sum_{k=1}^{\Upsilon_T}\bE\Big[\indic(\cE_T)\hat{\tau}^{(i)} + \indic(\cE_T) \sum_{t=\widehat{\tau}^{(i)}+1}^{\tau^{(i+1)}}\left(\mu_{k^*}^{(i)} - \mu_{A(t)}^{(i)}\right)\indic\left(\UCB_{A(t)}(t) \geq \mu_{k^*}^{(i)}\right)\Big] \\
    ...
    &\hspace{1cm}\bE\Big[\indic(\cE_T) \sum_{t=\hat{\tau}^{(i)}+1}^{\tau^{(i+1)}}\sum_{s=1}^{t-\widehat{\tau}^{(i)}}\indic\left(A(t) = i,n_k(t) = s\right) \indic\left(s \, \kl(\tilde{\mu}_{k,s}^{(i)}, \mu_{k^*}^{(i)}) \leq f(\tau^{(i+1)} - \widehat{\tau}^{(i)})\right)\Big]
\end{align*}%
\begin{align*}
    & \leq \sum_{k=1}^{\Upsilon_T}d_k^{(i)}+ \sum_{k=1}^K \sum_{k=1}^{\Upsilon_T}\left(\mu_{k^*}^{(i)} - \mu_k^{(i)}\right)\bE\Big[\indic(\cE_T) \sum_{s=1}^{n_k(\tau^{(i+1)})}\indic\left(s \, \kl(\tilde{\mu}_{k,s}^{(i)}, \mu_{k^*}^{(i)}) \leq f(\tau^{(i+1)} - \tau^{(i)})\right)\Big]  \\
    & \leq \sum_{k=1}^{\Upsilon_T}d_k^{(i)}+ \sum_{k=1}^K \sum_{k=1}^{\Upsilon_T}\left(\mu_{k^*}^{(i)} - \mu_k^{(i)}\right)\bE\Big[\indic(\cC^{(i)}) \sum_{s=1}^{n_k(\tau^{(i+1)})}\indic\left(s \, \kl(\tilde{\mu}_{k,s}^{(i)}, \mu_{k^*}^{(i)}) \leq f(\tau^{(i+1)} - \tau^{(i)})\right)\Big].
\end{align*}

Conditionally to $\cF_{\widehat{\tau}^{(i)}}$, when $\cC^{(i)}$ holds, for $s \in [n_k(\tau^{(i+1)})]$, $\tilde{\mu}_{k,s}^{(i)}$ is the empirical mean from \iid{} observations of mean $\mu_k^{(i)}$.
...
\[R_T \leq 2\sum_{k=1}^{\Upsilon_T} \frac{4K}{\alpha \left(\Delta^{(i)}\right)^2}\beta(T,\delta) +\alpha T + \delta (K+1)\Upsilon_T  + \sum_{k=1}^{\Upsilon_T}\sum_{i : \mu_k^{(i)} \neq \mu_{k^*}^{(i)}} \frac{\left( \mu_{k^*}^{(i)}-\mu_k^{(i)}\right)}{\kl\left(\mu_k^{(i)},\mu_{k^*}^{(i)}\right)}\ln(T) + \bigO{\sqrt{\ln(T)}},\]
which concludes the proof.


\subsubsection{Controlling the probability of the good event: proof of Lemma~\ref{lem:6:GoodEventGlobal}}

% We introduce $\widehat{\mu}_{k,s}^{(i)}$ as the empirical mean of the first $s$ observations of arm $k$ made after $t=\tau^{(i)}+1$ and $\tilde{\mu}_{k,s}^{(i)}$ as the empirical mean of the first $s$ observations of arm $k$ made after time $t=\hat{\tau}^{(i)}+1$.
...
%
\begin{align*}
    (a) & \leq \bP\left(\exists i, \exists s < t \leq n_k(\tau_k^{(i)}) : s \, \kl\left(\tilde{\mu}_{i,1:s}^{(i-1)},\tilde{\mu}_{i,1:t}^{(i-1)}\right) + (t - s) \, \kl\left(\tilde{\mu}_{i,s+1:t}^{(i-1)},\tilde{\mu}_{i,1:t}^{(i-1)}\right) > \beta(t, \delta) \;|\; \cC^{(i-1)}\right) \\
    & \leq \sum_{k=1}^K\bP\left(\exists s < t : s \, \kl(\widehat{\mu}_{1:s},\mu_k^{(i-1)}) + (t - s) \, \kl(\widehat{\mu}_{s+1:t}, \mu_k^{(i-1)}) > \beta(t, \delta)\right),
\end{align*}
with $\hat\mu_{s:s'} = \sum_{r=s}^{s'} Z_{i,r}$ where $Z_{i,r}$ is an \iid{} sequence with mean $\mu_k^{(i-1)}$.
Indeed, conditionally to $\cC^{(i-1)}$, the $n_k(\tau^{(i)})$ successive observations of arm $k$ arm starting from $\hat \tau^{(i)}$ are \iid{} with mean $\mu_k^{(i-1)}$.
...
%
From the definition of $\Delta^{(i)}$, there exists an arm $k$ such that $\Delta^{(i)} = |\mu_k^{(i)} - \mu_k^{(i-1)}|$. We shall prove that it is unlikely that the change-point detector associated to $i$ doesn't trigger within the delay $d^{(i)}$.

First, it follows from Proposition~\ref{prop:6:EnoughSamples} that there exists $\overline{t} \in \{\tau^{(i)}, \dots, \tau^{(i)} + d^{(i)} \}$ such that
$n_k(\overline{t}) - n_k(\tau^{(i)}) = \overline{r}$ where $\overline{r} = \lfloor \frac{\alpha}{K} d^{(i)}\rfloor$
...
\[
    n_k(\tau^{(i)}) \, \kl\left(\tilde{\mu}^{k-1}_{i,n_k(\tau^{(i)})},\tilde{\mu}^{k-1}_{i,n_k(\overline{t})}\right)
    + \overline{r} \, \kl\left(\tilde{\mu}^{k-1}_{i,n_k(\tau^{(i)}) : n_k(\overline{t})},\tilde{\mu}^{k-1}_{i,n_k(\overline{t})}\right) \leq \beta(n_k(\tau^{(i)}) + \overline{r},\delta),
\]
where $\tilde{\mu}^{k-1}_{k,s}$ denotes the empirical mean of the $s$ first observation of arm $k$ since the $(k-1)$-th restart $\hat{\tau}^{(i-1)}$ and  $\tilde{\mu}^{k-1}_{i,s:s'}$ the empirical mean that includes observation number $s$ to number $s'$. Conditionally to $\cC^{(i-1)}$, $\tilde{\mu}^{k-1}_{i,n_k(\tau^{(i)})}$ is the empirical mean of $n_k(\tau^{(i)})$ \iid{} replications of mean $\mu^{k-1}$, whereas $\tilde{\mu}^{k-1}_{i,n_k(\tau^{(i)}) : n_k(\overline{t})}$ is the empirical mean of $\overline{r}$ \iid{} replications of mean $\mu_k^{k}$.

Moreover, due to Proposition~\ref{prop:6:EnoughSamples}, $n_k(\tau^{(i)})$ lies in the interval $
\left[\left\lfloor \frac{\alpha}{K}\left(\tau^{(i)}-\widehat{\tau}^{(i-1)}\right)\right\rfloor,\left(\tau^{(i)}-\widehat{\tau}^{(i-1)}\right)\right]$.
...
%
\begin{eqnarray*}
    n_k(\tau^{(i)})& \in& \left\{ \left\lfloor \frac{\alpha}{2K} (\tau^{(i)}-\tau^{(i-1)} )\right\rfloor, \dots, \tau^{(i)}-\tau^{(i-1)} \right\} := \cI_k.
\end{eqnarray*}
...
\]
where we have also used that $\tilde{\mu}^{k-1}_{i,n_k(\overline{t})} = \left(n_k(\tau^{(i)})\tilde{\mu}^{k-1}_{i,n_k(\tau^{(i)})} + \overline{r}\tilde{\mu}^{k-1}_{i,n_k(\tau^{(i)}) : n_k(\overline{t})} \right) / (n_k(\tau^{(i)}) + \overline{r})$.

Using Pinsker's inequality from Lemma~\ref{lem:2:Pinsker} and introducing the gap $\Delta_k^{(i)} = \mu_k^{(i-1)} - {\mu}_k^{(i)}$ (which is such that $\Delta^{(i)} = |\Delta_k^k|$), one can write
%
\begin{align}
...
\end{equation}
%
Define $s_{\min} = \left\lfloor \frac{\alpha}{K} (\tau^{(i)}-\tau^{(i-1)})/2\right\rfloor$. Using that the mappings $s \mapsto (s+\overline{r})/s\overline{r}$ and $s \mapsto \beta(s + \overline{r},\delta)$ are respectively decreasing and increasing in $s$, one has, for all $s\in \cI_k$,
\begin{eqnarray*}
    2\frac{s+\overline{r}}{s\overline{r}}\beta\left(s+\overline{r}, \delta\right) & \leq &
    2\frac{s_{\min}+\overline{r}}{s_{\min}\overline{r}}\beta\left(T, \delta\right) \leq \frac{4\beta(T,\delta)}{\left\lfloor \frac{\alpha}{K}d^{(i)}\right\rfloor},
\end{eqnarray*}
%
...
\[\left\lfloor \frac{\alpha}{K}d^{(i)}\right\rfloor > \frac{4\beta(T,\delta)}{\left(\Delta^{(i)}\right)^2},\]
which yields
...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=0.90\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_5.pdf}
...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=0.90\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_4.pdf}
...
\begin{figure}[h!]  % [htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{2-Chapters/6-Chapter/nonstatbandits.git/figures/6_Problems/Problem_6.pdf}

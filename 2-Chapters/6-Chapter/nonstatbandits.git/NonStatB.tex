...
\begin{figure}[h!]  % [htbp]
    ...
    \subcaption{\textbf{Pb $1$}: $K=3$ arms with $T=5000$, and $\Upsilon=4$ changes occur on only one arm at a time (\ie, $C=4$).}
...
%
Thus it is fair to compare our proposal against the modified versions of the two algorithms CUSUM and M- that use the \klUCB{} index policy instead of \UCB, as they perform uniformly better with \klUCB{} than with \UCB{} (the same tendency was observed on all other problems).
...
\begin{table}[ht]
  % \begin{footnotesize}
      \centering
      \begin{tabular}{c|cc}
        \textbf{Algorithm} $\;$ \textbackslash $\;$ Problems & Pb 1 & Pb 2 \\
        \hline
        Oracle-Restart \UCB{} & $216$ & $67$ \\
        Oracle-Restart \klUCB{} & $\mathbf{54}$ & $\mathbf{219}$ \\
        \hline
        \MUCB{} & $878$ & $2040$ \\
        \MklUCB{} & $817$ & $1485$ \\
        \hline
        ...
        \CUSUMklUCB{} & $304$ & $316$ \\
        \hline
        \GLRklUCB{}(Local) & $\mathbf{132}$ & $\mathbf{186}$
      \end{tabular}
...
The Exp3.S algorithm was found to outperform other algorithms based on Exp3, including Exp3, and recent variants like Exp3$++$ \cite{Seldin17} or Exp3.R from \cite{Allesiardo17},
...
% \begin{footnotesize}
    ...
    \textbf{Algorithms} $\;$ \textbackslash $\;$ \textbf{Problems} & Pb $1$ & Pb $2$ & Pb $3$ \\
        ...
        \hline
        Exp3.S & $352 \pm 51$ & $310 \pm 62$ & $XXX$ \\
        \hline
...
\begin{table}[ht]
    ...
    \begin{tabular}{c|cccccc}
        ...
        \hline
        Exp3.S & $551 \pm 63$ & $860 \pm 109$ & $XXX$ \\
        \hline
...
Their algorithm assume that player $j$ knows its ID $j\in\{1,\dots,M\}$ as well as $\nu$, and removing these hypotheses is an interesting direction of future work.

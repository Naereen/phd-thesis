...
Using Pinsker's inequality from Lemma~\ref{lem:2:Pinsker} and introducing the gap $\Delta_k^{(i)} \eqdef \mu_k^{(i-1)} - {\mu}_k^{(i)}$ (which is such that $\Delta^{(i)} = |\Delta_k^k|$), one can write
%
\begin{align}

...
We introduce the model and review related works in Section~\ref{sec:6:BanditSetting}. In Section~\ref{sec:6:ChangePointDetector}, we study the Generalized Likelihood Ratio test (GLRT) as a Change-Point Detector (CPD) algorithm.
...
We define the $k$-th breakpoint by
\begin{equation}
    \tau^{(k)} = \inf\{t > \tau^{(k-1)} : \exists i : \mu_i(t) \neq \mu_{i}(t+1)\}.
\end{equation}
%
Hence for $t\in[\tau^{(k)} + 1,\tau^{(k+1)}]$, the rewards $(X_{i,t})$ associated to each arms are \iid.
...
\paragraph{An interesting generalization}
%
...
Using the form of the likelihood for Bernoulli distribution, this statistic can be written with the binary relative entropy $\kl$, defined for $x,y\in[0,1]$ (with the usual convention that $t \log(t) = 0$ if $t=0$).
%
...
In this section, we provide new non-asymptotic results about the Bernoulli GLR test under the assumption that the samples $(X_t)$ come from a sub-Bernoulli distribution, which holds for any distribution supported in $[0,1]$.
...
% ----------------------------------------------------------------------------
\subsection{Non-Asymptotics Properties of the Bernoulli GLR}\label{subsec:6:PropGLR}
...
%
We give below two Lemmas, Lemma~\ref{lem:6:FalseAlarm} which bounds the false alarm probability for a choice of threshold function, and Lemma~\ref{lem:6:DetectionDelay} which bounds the detection delay if there is enough samples before a change.
...
\begin{lemma}\label{lem:6:FalseAlarm}
    ...
    $\bP_{\mu_0}(\hat\tau_\delta < \infty) \leq \delta$, with the threshold function
    %
...
For a threshold $\beta$ chosen as in the Lemma~\ref{lemma:FalseAlarm}, we can show that a consequence of the Lemma~\ref{lemma:Delay} is that if the break $\tau$ takes place after $\Delta^{-2} \ln(1/\delta)$ samples, the detection time may be of the same magnitude.
%
In other words,
if the B-GLRT test observes enough samples before rupture,
its delay $\widehat{\tau}_\delta$ is in the order of $O(\Delta^{-2} \ln(1/\delta))$, in high probability.

...
\end{equation}
%
Indeed, one can show that $\GLR(n) = \sup_{s \in [2,n-1]} \left[s \times \kl\left(\widehat{\mu}_{1:s},\widehat{\mu}_{1:n}\right) + (n-s) \times \kl\left(\widehat{\mu}_{s+1:n},\widehat{\mu}_{1:n}\right)\right]$,
...
% ----------------------------------------------------------------------------
\subsection{Non-Asymptotics Properties of the B-GLRT}\label{subsec:6:PropGLR}
...
The proof of Lemma~\ref{lem:6:FalseAlarm}, that actually holds for any sub-Bernoulli distribution, is given in Appendix~\ref{proof:6:FalseAlarm}.
...
\[\widehat{\mu}_i(t) = (1/n_i(t)) \sum_{s=\tau_i(t)+1}^{t}X_{i,s} \indic(A_s=i)\]
...
%-----------------------------------------------------------------------------
\subsection{A Unified Regret Analysis}
...
\begin{align*}
    ...
    \\ & \leq \sum\limits_{k=1}^{\Upsilon_T} \underbrace{\bP\left(\widehat{\tau}^{(k)} \leq \tau^{(k)} \;|\; \cC^{(k-1)}\right)}_{(a)} + \sum\limits_{k=1}^{\Upsilon_T} \underbrace{\bP\left(\widehat{\tau}^{(k)} \geq \tau^{(k)} + d^{(k)} \;|\; \cC^{(k-1)}\right)}_{(b)}.
\end{align*}
%
...
%
$\widehat{\tau}^{(k)} \leq \tau^{(k)}$ implies that there exists an arm whose associated change point detector has experienced a false-alarm:
%
\begin{align*}
...
\paragraph{Illustrations of our benchmark.}\label{app:6:Figures}

We give below the plots of the cumulated mean regret $R_t$ (as a function of $t$), as well as plots of histograms of the distribution of final regret $R_T$, for the considered algorithms.
...
In this chapter, we studied the piece-wise stationary bandit model, and reviewed existing work.
